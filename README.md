...
- If the target path already contains a dirty checkout or other non-empty content, the remote bootstrap now moves it aside into a timestamped `*-backup-*` directory and then performs a fresh clone instead of hard-failing on `git pull --ff-only`.

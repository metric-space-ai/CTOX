...
- The installer now auto-detects a non-loopback host IP, prefers a Tailnet/VPN IPv4 when available, and writes `CTO_AGENT_BIND_HOST=0.0.0.0` plus `CTO_AGENT_PUBLIC_BASE_URL=https://<detected-ip>:8443` into `runtime/kleinhirn.env` by default.
- If you want to override that, set `CTO_AGENT_BIND_HOST`, `CTO_AGENT_PUBLIC_BASE_URL`, or `CTO_AGENT_TLS_ALT_NAMES` before install or re-install, or change them later on the attach TUI Settings page. The runtime restarts automatically after those BIOS network settings are saved.

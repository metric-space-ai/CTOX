...
pub(crate) async fn run_compaction_controller(
    ...
    let progress_review = if blocks.is_empty() {
        ProgressStageResponse {
            summary: "Kein verwertbarer Kontext vorhanden.".to_string(),
            school_grade: 4,
            rationale: "Ohne nutzbaren Verlauf ist der Fortschritt nur ausreichend bewertbar."
                .to_string(),
        }
    } else {
        loop {
    ...
    if !blocks.is_empty() {
        ...
        for iteration_index in 1..=DEFAULT_ITERATIONS {
            ...
            let real_change = after_chars != previous_chars || decisions.iter().any(|item| item.changed);
    ...
    let output_result = if story_blocks.is_empty() && anchor_blocks.is_empty() && focus_blocks.is_empty()
    {
        OutputStageResponse {
            story_title: "Kontinuitaetsnarrativ".to_string(),
            story_draft: String::new(),
            anchor_title: "Kontinuitaetsanker".to_string(),
            anchor_draft: String::new(),
            focus_title: "Aktiver Fokus".to_string(),
        ...
        run_structured_prompt::<OutputStageResponse>(sess, turn_context, prompt, output_schema()).await?
    };
...
fn build_screen_prompt(
    ...
    let serialized = serde_json::to_string_pretty(&serialize_blocks_for_prompt(blocks)).map_err(
        |err| CodexErr::InvalidRequest(format!("failed to serialize screen blocks: {err}")),
    )?;
    Ok([
        "Du destillierst einen langen Arbeitskontext in drei Produkte.".to_string(),
        "Produkt 1: Kontinuitaetsnarrativ.".to_string(),
        "Das ist eine kurze Ursache-Wirkung-Geschichte: Ausgangslage, erkannte Fehlursache, wichtige Wendepunkte, dauerhafte Entscheidungen und warum die Regeln weiter gelten.".to_string(),
        "Produkt 2: Kontinuitaetsanker.".to_string(),
        "Das sind harte Dinge, die nicht rausgewaschen werden duerfen: IDs, Skripte, Hosts, Artefakte, Gates, Invarianten, Verbote, Pruefpfade.".to_string(),
        "Produkt 3: Aktiver Fokus.".to_string(),
        "Das ist nur der aktuelle Arbeitsrand: Status, Blocker, naechster Schritt, Done-/Gate-Bedingung.".to_string(),
        "Bearbeite jeden Block genau einmal.".to_string(),
        "action:".to_string(),
        "- keep = unveraendert behalten".to_string(),
        "- compress = inhaltlich behalten, aber kuerzen".to_string(),
        "- drop = sicher verwerfen".to_string(),
        "bucket/route:".to_string(),
        "- story = nur Narrativ".to_string(),
        "- anchor = nur Anker".to_string(),
        "- focus = nur Fokus".to_string(),
        "- story_anchor = Narrativ + Anker".to_string(),
        "- story_focus = Narrativ + Fokus".to_string(),
        "- anchor_focus = Anker + Fokus".to_string(),
        "- all = in alle drei Produkte".to_string(),
        "- discard = in keines der drei Produkte".to_string(),
        "Regeln:".to_string(),
        "1. Erfinde keine Fakten.".to_string(),
        "2. Wenn etwas spaeter teuer waere zu verlieren, gehoert es eher in story oder anchor als in discard.".to_string(),
        "3. Chat-Rauschen, UI-Rauschen, Statusspam, triviale Zwischenmeldungen und wiederholte Formulierungen duerfen raus.".to_string(),
        "4. Wenn ein Block den roten Faden traegt, aber zu breit ist, nutze compress statt drop.".to_string(),
        "5. revised_text nur bei compress fuellen. Sonst leer lassen.".to_string(),
        "6. Wenn action drop ist, setze bucket auf discard.".to_string(),
        format!(
            "7. Das Reservoir nach dieser Phase soll grob auf etwa {reservoir_target_chars} Zeichen oder darunter schrumpfen, ohne Kausalitaet oder Recovery-Anker zu verlieren."
        ),
        format!(
            "8. Die finalen drei Produkte zusammen sollen spaeter auf etwa {final_target_chars} Zeichen kommen."
        ),
        String::new(),
        "<NEXT_STEP>".to_string(),
        if task.is_empty() {
...
fn build_progress_prompt(
    ...
    let serialized = serde_json::to_string_pretty(&serialize_blocks_for_prompt(blocks)).map_err(
        |err| CodexErr::InvalidRequest(format!("failed to serialize progress blocks: {err}")),
    )?;
    Ok([
        "Bewerte den bisherigen Agentenfortschritt fuer diesen Arbeitskontext in deutschen Schulnoten.".to_string(),
        "Note 1 = sehr gut, klarer Fortschritt mit substanzieller Bewegung.".to_string(),
        "Note 6 = ungenuegend, keine wirksame Bewegung, Schleife, Sackgasse oder chaotische Lage.".to_string(),
        "Bewerte hart und konservativ.".to_string(),
        "Regeln:".to_string(),
        "1. Erfinde keine Fakten.".to_string(),
        "2. Gute Noten nur bei echter Bewegung: Entscheidungen, verifizierte Teilfortschritte, aufgeloeste Blocker, sauberer naechster Schritt.".to_string(),
        "3. Schleifen, Wiederholungen, diffuse Lage, fehlende Wirksamkeit oder wachsende Verwirrung verschlechtern die Note.".to_string(),
        "4. summary bleibt kurz. rationale nennt den Hauptgrund fuer die Note.".to_string(),
        String::new(),
        "<CURRENT_MODEL>".to_string(),
        current_model.to_string(),
        "</CURRENT_MODEL>".to_string(),
...
fn build_iteration_prompt(
    ...
    let serialized = serde_json::to_string_pretty(&serialize_blocks_for_prompt(blocks)).map_err(
        |err| CodexErr::InvalidRequest(format!("failed to serialize iteration blocks: {err}")),
    )?;
    Ok([
        "Du verfeinerst ein bereits vorgefiltertes Kontext-Reservoir fuer drei Produkte: Kontinuitaetsnarrativ, Kontinuitaetsanker und Aktiver Fokus.".to_string(),
        format!("Aktuelle Iteration: {iteration_index}."),
        format!(
            "Aktuell sind etwa {current_chars} Zeichen im Reservoir. Ziel sind etwa {reservoir_target_chars} Zeichen oder weniger, ohne spaeter teure Wissensverluste zu erzeugen."
        ),
        "Bearbeite jeden Block genau einmal.".to_string(),
        "Regeln:".to_string(),
        "1. Erfinde keine Fakten.".to_string(),
        "2. Halte Narrativ, Anker und Fokus getrennt. Nicht alles Dauerhafte gehoert in den Fokus, und nicht jeder Anker gehoert in die Geschichte.".to_string(),
        "3. Du darfst einen Block zwischen story, anchor, focus und Kombi-Routen verschieben, wenn das die Trennung verbessert.".to_string(),
        "4. Bei Wiederholungen soll nur die staerkere oder kompaktere Version ueberleben.".to_string(),
        "5. compress darf nur lokal verdichten. Keine neuen Behauptungen.".to_string(),
        "6. Wenn action drop ist, muss bucket discard sein.".to_string(),
        "7. Behalte Wendepunkte und Ursache-Wirkung eher im story-Topf, harte IDs/Artefakte/Gates eher im anchor-Topf.".to_string(),
        "8. Wenn der Zielwert schon fast erreicht ist, priorisiere Klarheit und saubere Zuordnung statt blinder weiterer Kuetzung.".to_string(),
        String::new(),
        "<NEXT_STEP>".to_string(),
        if task.is_empty() {
            "(leer)".to_string()
...
fn build_output_prompt(
    ...
    let story_serialized =
        serde_json::to_string_pretty(&serialize_blocks_for_prompt(story_blocks)).map_err(|err| {
            CodexErr::InvalidRequest(format!("failed to serialize story blocks: {err}"))
        })?;
    let anchor_serialized = serde_json::to_string_pretty(&serialize_blocks_for_prompt(anchor_blocks))
        .map_err(|err| CodexErr::InvalidRequest(format!("failed to serialize anchor blocks: {err}")))?;
    let focus_serialized =
        serde_json::to_string_pretty(&serialize_blocks_for_prompt(focus_blocks)).map_err(|err| {
            CodexErr::InvalidRequest(format!("failed to serialize focus blocks: {err}"))
        })?;

    Ok([
        "Erzeuge drei knappe Outputs aus dem gefilterten Kontext.".to_string(),
        "Output A: Kontinuitaetsnarrativ.".to_string(),
        "Output B: Kontinuitaetsanker.".to_string(),
        "Output C: Aktiver Fokus.".to_string(),
        "Regeln:".to_string(),
        "1. Erfinde keine Fakten.".to_string(),
        "2. story soll eine kurze, robuste Verlaufsgeschichte sein, damit bei spaeterer Rekompaktierung der rote Faden nicht verloren geht.".to_string(),
        "3. anchor soll harte, dichte Anker enthalten: IDs, Skripte, Hosts, Artefakte, Invarianten, Gates, Pruefpfade.".to_string(),
        "4. focus soll nur den unmittelbar aktiven Arbeitsrand enthalten: Status, Blocker, Next, Done.".to_string(),
        "5. Wenn etwas in mehreren Outputs auftaucht, halte die Doppelung klein.".to_string(),
        format!(
            "6. Alle drei Outputs zusammen sollen ungefaehr {final_target_chars} Zeichen oder weniger haben."
        ),
        "7. story darf in klarer Prosa schreiben. anchor darf knapper und listenartiger sein. focus soll brutal knapp bleiben.".to_string(),
        "8. Keine Markdown-Codefences.".to_string(),
        String::new(),
        "<NEXT_STEP>".to_string(),
        if task.is_empty() {
            "(leer)".to_string()
...
fn build_reprioritization_prompt(
    ...
    Ok([
        "Du pruefst nach der Kontextkompaktierung die Aufgabenpriorisierung fuer einen leichten Wrapper.".to_string(),
        "Baue keinen konkurrierenden Agenten-Loop. Der Wrapper soll nur wissen, was jetzt priorisiert weiterlaufen soll.".to_string(),
        "Wenn ein Interrupt diese Kompaktierung ausgeloest hat, behandle das als Repriorisierungssignal.".to_string(),
        "In diesem Core-Pfad liegen bei einem Interrupt nicht immer konkrete Zusatzdetails vor. Erfinde deshalb keine Interrupt-Inhalte; nutze nur das Signal, dass eine neue Priorisierung noetig sein kann, plus den sichtbaren Kontext.".to_string(),
        "Regeln:".to_string(),
        "1. Erfinde keine Fakten.".to_string(),
        "2. active_task benennt genau die Aufgabe oder das Task-Paket, das jetzt vorn liegt.".to_string(),
        "3. task_packet bleibt klein und konkret. Keine breite Roadmap.".to_string(),
        "4. completed_tasks nennt Aufgaben, die jetzt als erledigt in den Wrapper gespiegelt werden sollen.".to_string(),
        "5. spawned_tasks sind neue kleine Folgeaufgaben mit title, detail und priority_bucket(now|next|later).".to_string(),
        "6. mutated_tasks passen bestehende Aufgaben an. action ist eines von complete, revise, split, drop, defer.".to_string(),
        "7. priority_order ordnet die wichtigsten Aufgaben oder Task-Pakete in Reihenfolge.".to_string(),
        "8. Wenn die Priorisierung wegen Interrupt oder neuer Lage neu geprueft werden muss, setze should_reprioritize auf true.".to_string(),
        "9. next_action muss eines von vier Labels sein: continue_current_task, reprioritize, switch_task, park_current_task.".to_string(),
        "10. interrupts_reviewed ist true, wenn du das Interrupt-Signal in deiner Bewertung beruecksichtigt hast.".to_string(),
        "11. Keine Markdown-Codefences.".to_string(),
        String::new(),
        "<TRIGGER>".to_string(),
...
fn retained_blocks(blocks: &[CompactionBlock]) -> Vec<CompactionBlock> {
    blocks.iter().filter(|block| !block.dropped).cloned().collect()
}
...
fn apply_decisions(
    ...
    for block in blocks.iter_mut() {
        ...
        let next_text = coerce_revised_text(
            action,
            incoming.map(|item| item.revised_text.as_str()).unwrap_or(""),
            &previous_text,
        );
        let bucket = normalize_bucket(
            action,
            incoming.map(|item| item.bucket.as_str()).unwrap_or(block.bucket.as_str()),
            &block.bucket,
        );
...
async fn run_structured_prompt<T: DeserializeOwned>(
    ...
    loop {
        let attempt = run_structured_prompt_once::<T>(sess, turn_context, &mut client_session, &prompt).await;
        match attempt {
...
mod tests {
    ...
    fn segment_context_splits_markdown_sections_into_blocks() {
        ...
        let titles = blocks.iter().map(|block| block.title.as_str()).collect::<Vec<_>>();

...
| 2026-07-28 | Kimi | F (F-010 layer 2) | `src/core/service/service.rs`, `HARNESS.md`, `src/skills/system/research/systematic-research/SKILL.md` | Completion validation recognizes `exec_command`/`shell` invocations of `ctox web search|scholarly search|deep-research|read` as equivalents of the typed discovery/read tools (the typed `CtoxWebHandler` spawns the same CLI — same server-owned receipts, same rollout binding to run/command/workspace). Fail-closed strict parser: single plain invocation only; operators/pipes/redirects/substitutions unrecognizable (tokenizer treats `;\|&<>` as boundaries even quote-adjacent). Envelope extraction handles exec output wrappers (`Wall time:`/`Output:` sections). Receipts gain `transport: typed_tool|exec_cli`; observed-call notes carry `via exec_command`. 3 new tests: CLI discovery receipt (exhaustive, incl. echo-fabrication + operator rejection), CLI coverage rounds (2 ok + 1 failed, scholarly counting), CLI web-read receipts (incl. redirect rejection). Root cause: queue worker used only exec_command (165:0) — contract was structurally unsatisfiable; F-R5. | `cargo check --bin ctox` clean (2m24s, 2026-07-28, after overnight build-lock contention + foreign rxdb WIP compile errors cleared); `cargo test --bin ctox systematic_research`: 28/28 incl. 3 new exec-CLI tests; `cargo fmt --check` clean | `974b7a6ea` (hunk-split via temp index; concurrent foreign WIP excluded) | `origin/main` (pushed 2026-07-28, FF `860eb38e9..45b4f052a`) |

...
cleanup() {
  ...
  pkill -f "$ROOT/ctox-vllm-serve/target/release/mistralrs-server" 2>/dev/null || true
  pkill -f "$ROOT/ctox-vllm-serve/target/release/mistralrs-cli" 2>/dev/null || true
...
./target/release/ctox chat-runtime-apply-legacy "Qwen/Qwen3.5-35B-A3B" >/tmp/ctox_qwen35b_legacy_apply.json
source "$ROOT/runtime/vllm_serve.env"

export CTOX_VLLM_SERVE_PORT="$PORT"
export CTOX_VLLM_SERVE_TOPOLOGY="$TOPOLOGY_PATH"
if [[ -n "$DEVICE_LAYERS_SPEC" ]]; then
  DEVICE_LAYERS_SPEC="${DEVICE_LAYERS_SPEC//,/;}"
  export CTOX_VLLM_SERVE_DEVICE_LAYERS="$DEVICE_LAYERS_SPEC"
  export CTOX_VLLM_SERVE_ALLOW_DEVICE_LAYERS_WITH_TOPOLOGY=1
else
  unset CTOX_VLLM_SERVE_DEVICE_LAYERS
  unset CTOX_VLLM_SERVE_DEVICE_LAYERS_CLI
  unset CTOX_VLLM_SERVE_ALLOW_DEVICE_LAYERS_WITH_TOPOLOGY
fi
...
set +e
timeout 900 ./scripts/run_vllm_serve_backend.sh >"$LOG_PATH" 2>&1 &
LAUNCH_PID=$!
set -e

...
fn render_loop_repair_prompt(
    ...
    let mut lines = vec![
        "Repair the mission loop before CTOX retries the same tactic again.".to_string(),
        String::new(),
        "Goal:".to_string(),
        goal.trim().to_string(),
        String::new(),
        format!("Mission state: {mission_status}"),
        format!("Repeated blocker: {}", blocker.trim()),
        format!("Previous next-slice hint: {}", next_slice.trim()),
        format!("Done gate: {}", done_gate.trim()),
        String::new(),
        "Governor rules:".to_string(),
        "- Do not retry the same tactic unless you first produce new evidence that changes the blocker.".to_string(),
        "- Reconstruct the minimum mission contract: real status, blocker, next bounded slice, and done gate.".to_string(),
        ...
        "- If the blocker is internal, choose one materially different next slice or replan the work into smaller slices.".to_string(),
    ];
    ...
    lines.push(String::new());
    lines.push("Return a durable outcome, not another vague continuation note.".to_string());
    lines.join("\n")
}

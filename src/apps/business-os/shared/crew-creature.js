...
export const CREATURE_SHAPES = Object.freeze([
  "round",
  "square",
  "triangle",
  "blob",
...
export function creatureState({
  ...
  if (!task)
    return member?.state === "resting_after_failure"
      ? "failed"
      : member?.state === "on_duty"
        ? "waking"
        : "sleeping";
  const status = String(task.status || task.task_status || "").toLowerCase();
  const route = String(
    task.route_status || task.routeStatus || "",
  ).toLowerCase();
  const phase = String(
    task.execution_phase || task.executionPhase || "",
  ).toLowerCase();
  const review = String(
    task.execution_progress?.review?.status ||
      task.executionProgress?.reviewStatus ||
      "",
  ).toLowerCase();
  if (
    FAILED.has(status) ||
    (phase === "terminal" && FAILED.has(task.terminal_status))
  )
    return "failed";
  if (
    DONE.has(status) ||
    (phase === "terminal" && DONE.has(task.terminal_status))
  )
    return "done";
  if (
    ["blocked", "cancelled", "canceled"].includes(status) ||
    ["blocked", "deferred"].includes(route) ||
    (task.retry_not_before && Date.parse(task.retry_not_before) > now) ||
    task.wait_entity_id
  )
    return "waiting";
  if ([status, route, phase, review].some((value) => REVIEW.has(value)))
    return "reviewing";
  ...
  const recent = events
    .filter(
      (event) =>
        ...
        now - event.created_at_ms <= 10_000,
    )
    .sort(
      (a, b) =>
  ...
  for (const event of recent) {
    ...
    if (["thinking", "plan_updated", "tool_completed"].includes(event.kind))
      return "thinking";
...
const eyes = Object.freeze({
  ...
  reviewing:
    '<circle cx="24" cy="35" r="6"/><circle cx="41" cy="35" r="6"/><path d="M30 35h5m-7 12h8"/>',
  waiting: '<path d="M24 32v5m16-5v5m-12 9h8"/>',
  failed:
    '<path d="M19 29l10 12m0-12L19 41m16-12l10 12m0-12L35 41m-7 8q4-5 8 0"/>',
...
const symbols = Object.freeze({
  ...
  waiting:
    '<path d="M48 6h13m-13 19h13M49 7q0 7 6 9-6 2-6 8m11-17q0 7-5 9 5 2 5 8"/>',
...
export function crewCreatureHtml(
  member,
  { state = "sleeping", size = "home", label = "", decorative = false } = {},
) {
  if (
    !member?.id ||
    !CREATURE_SHAPES.includes(member.shape) ||

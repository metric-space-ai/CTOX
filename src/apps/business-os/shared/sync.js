commit d5e8557843a0ddd1c16478c6a0c817682c22d76f
Author: Codex <codex@openai.com>
Date:   Tue Jul 28 14:42:57 2026 +0200

    business-os(shared): restore sync.js and db.js from commit-message overwrite
    
    Commit 12e1f0fd4 replaced the contents of both files with the text of a
    git commit message — sync.js went from 2832 lines to 91, db.js from 382
    to 91, each starting with a literal "commit 9a9e043f2cd0…" header. That
    is not a refactor, it is a redirect accident, and it shipped: the commit
    is on origin/main, so the pushed history cannot boot the Business OS
    browser runtime at all. sync.js is the sync runtime; db.js opens the
    database.
    
    Both files are restored from e5254fbfb, the last revision where they
    were intact. The restore is consistent with the legitimate half of
    12e1f0fd4 (the RTC admission removal in webrtc-native.mjs): neither file
    references any of the symbols that commit deleted —
    MAX_GLOBAL_RTC_PEER_CONNECTIONS, GLOBAL_RTC_CONNECTION_POOL_KEY,
    preemptOptionalRtcPeerConnectionSlot, RTC_CONNECTION_QUEUE_TIMEOUT_MS,
    criticalRtcPeerConnectionsReady — so no dangling references come back.
    
    dist had already been rebuilt for the admission removal, but the restored
    files still pointed at the pre-B4 cache buster, which would have served a
    stale bundle to any browser holding v85. All three import sites now carry
    20260728-rtc-admission-removal-v86.
    
    Measured, not assumed: the corrupt tree runs 68 passed / 22 failed; the
    restored tree runs 76 passed / 14 failed. The failure set is a strict
    subset — eight smokes recover (data-plane-guard, module-sync-lifecycle,
    sync-diagnostics-throttle, sync-profile-registry, checkpoint-age,
    command-plane-observability, error-classification-corpus,
    initial-replication-watchdog) and not one new failure appears.
    
    Not done here on purpose: installSignalingErrorObserver comes back with
    the restore. Removing the WebSocket monkeypatch was part of B4's stated
    scope, but the overwrite destroyed the evidence of what that session
    actually intended, and guessing it would be worse than leaving working
    code in place. B4 should finish that deliberately.
    
    Backlog: SYNC-A-B4-REPAIR
    
    Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>

diff --git a/src/apps/business-os/shared/db.js b/src/apps/business-os/shared/db.js
index cabd54efa..1b03414bc 100644
--- a/src/apps/business-os/shared/db.js
+++ b/src/apps/business-os/shared/db.js
@@ -1,91 +1,382 @@
...
+const RXDB_BUNDLE_URL = '../rxdb/dist/ctox-rxdb-js.mjs?v=20260728-rtc-admission-removal-v86';
+
...
+  }
+}
diff --git a/src/apps/business-os/shared/sync.js b/src/apps/business-os/shared/sync.js
index cabd54efa..b7ee4a496 100644
--- a/src/apps/business-os/shared/sync.js
+++ b/src/apps/business-os/shared/sync.js
@@ -1,91 +1,2832 @@
-commit 9a9e043f2cd0e6bf2ffa305a988adbb5e3d1f371
-Author: Codex <codex@openai.com>
-Date:   Tue Jul 28 10:45:36 2026 +0200
...
+    const rxdb = db?.rxdb || await import('../rxdb/dist/ctox-rxdb-js.mjs?v=20260728-rtc-admission-removal-v86');

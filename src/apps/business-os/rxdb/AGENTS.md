...
2. **Never patch `dist/ctox-rxdb-js.mjs` directly.** It is built from
   ...
   ```
   and bump the sole bundle `?v=` in `shared/rxdb-runtime.js`. Both
   `shared/db.js` and `shared/sync.js` import that loader with the same literal
   `?v=<APP_BUILD>` (a mismatch loads separate module graphs). Keep these
   loader busters equal to `APP_BUILD`; the data-plane guard checks them.
   App modules receive the database through the shell facade.

...
assert.match(js, /rowMajorGridPosition\(index/u);
assert.match(js, /reservedRects: desktopReservedRects\(\)/u);
assert.match(js, /ROW_MAJOR_LAYOUT_MIGRATION = 'row-major-v3-widget-reserved'/u);
...
assert.match(js, /normalizePosition: \(position, iconSize\)/u);

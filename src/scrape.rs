...
const DEFAULT_EMBEDDING_PORT: &str = "1237";
const DEFAULT_PROXY_PORT: &str = "12434";
...
fn execute_scrape(root: &Path, args: &[String]) -> Result<()> {
    ...
    let conn = open_db(root)?;
    let target = load_registered_target(&conn, target_key)?.context("target_key not found")?;
...
fn default_llm_enrichment_config(root: &Path, _target: &ScrapeTargetView) -> EnrichmentConfig {
    EnrichmentConfig {
        enabled: false,
        model: runtime_env::effective_chat_model(root)
...
fn load_registered_target(conn: &Connection, target_key: &str) -> Result<Option<RegisteredTarget>> {
    ...
    let script = conn
        ...
        .optional()?;
    Ok(script.map(|script| RegisteredTarget { view, script }))
}
...
fn execute_registered_script(
    ...
    child
        .args(&args)
        .current_dir(resolve_workspace_dir(
            Path::new(""),
            &target.view.workspace_dir,
        ))
        .env("CTOX_SCRAPE_TARGET_KEY", &target.view.target_key)
        .env("CTOX_SCRAPE_TARGET_DIR", &target.view.workspace_dir)
        .env(
            "CTOX_SCRAPE_MANIFEST_PATH",
            resolve_workspace_dir(Path::new(""), &target.view.workspace_dir)
                .join("manifest.json")
                .to_string_lossy()
                .to_string(),
        )
...
fn invoke_responses_text(
    ...
    let base_url =
        runtime_env::env_or_config(root, "CTOX_RESPONSES_BASE_URL").unwrap_or_else(|| {
            format!(
                "http://{}:{}",
                host,
                runtime_env::env_or_config(root, "CTOX_PROXY_PORT")
...
fn embed_texts(root: &Path, inputs: &[String], model: &str) -> Result<Vec<Vec<f64>>> {
    ...
    let base_url =
        runtime_env::env_or_config(root, "CTOX_EMBEDDING_BASE_URL").unwrap_or_else(|| {
            format!(
                "http://127.0.0.1:{}",
                runtime_env::env_or_config(root, "CTOX_EMBEDDING_PORT")
                    .unwrap_or_else(|| DEFAULT_EMBEDDING_PORT.to_string())
            )
        });

...
pub fn render_prompt_block(snapshot: &GovernancePromptSnapshot) -> String {
    let visible_mechanisms = snapshot
        ...
        .collect::<Vec<_>>();
    let mut lines = vec![
        "Survival control plane:".to_string(),
        "Only `survival` and `safety` mechanisms may act autonomously in the background. `advisory` mechanisms may influence your reasoning but must not silently mutate workflow state. `explicit_tool` mechanisms run only when you invoke them.".to_string(),
        "Known background governance mechanisms:".to_string(),
    ];
    ...
    if snapshot.recent_events.is_empty() {
        lines.push("Recent autonomous actions: none recorded recently.".to_string());
    } else {
        lines.push("Recent autonomous actions:".to_string());

...
impl SearchConfig {
    fn from_root(root: &Path) -> Self {
        Self {
            ...
            provider: ProviderKind::from_env(runtime_env::env_or_config(
                root,
                "CTOX_WEB_SEARCH_PROVIDER",
            )),
            searxng_base_url: runtime_env::env_or_config(root, "CTOX_WEB_SEARCH_SEARXNG_BASE_URL"),
            ...
            user_agent: runtime_env::env_or_config(root, "CTOX_WEB_SEARCH_USER_AGENT")
                .unwrap_or_else(|| {
            ...
            default_language: runtime_env::env_or_config(root, "CTOX_WEB_SEARCH_LANGUAGE"),
            default_region: runtime_env::env_or_config(root, "CTOX_WEB_SEARCH_REGION"),
...
pub fn augment_responses_request(
    ...
    let include_sources = request_includes_sources(payload);
    let Some(mut tool_request) = extract_web_search_request(payload) else {
        return Ok(None);
    };
    tool_request.include_sources = include_sources;
    ...
    let Some(query_text) = extract_latest_user_query(payload) else {
        return Ok(None);
    };
    ...
    let augmentation = match execute_search(root, &config, &tool_request, &query) {
        Ok(result) => {
            inject_developer_context(
                payload,
                render_results_context(&query_text, &tool_request, context_size, &result),
            );
        ...
        Err(err) => {
            inject_developer_context(
                payload,
                render_failure_context(&query_text, &tool_request, &err),
            );
            WebSearchAugmentation::search_failure(call_id, query_text, query_variants)
        }
    };
...
fn execute_search(
    ...
    let planned_queries = plan_search_queries(&query.text, &tool_request.allowed_domains);
    ...
    let mut response = search_with_query_plan(config, query, &planned_queries)?;
...
fn search_with_query_plan(
    ...
    for query_text in planned_queries {
        ...
        let response = match config.provider {
            ProviderKind::Google => google_search(config, &query),
...
fn google_search(config: &SearchConfig, query: &SearchQuery) -> Result<SearchResponse> {
    ...
    let mut url = Url::parse("https://www.google.com/search").expect("static google URL");
    {
        let mut qp = url.query_pairs_mut();
        qp.append_pair("q", &query.text);
        qp.append_pair("num", &query.count.min(10).to_string());
        qp.append_pair("start", &query.offset.to_string());
        if let Some(language) = query.language.as_deref() {
            qp.append_pair("hl", language);
        }
        if let Some(region) = query.region.as_deref() {
            qp.append_pair("gl", region);
        }
    ...
    if final_url.contains("/sorry/") || final_url.contains("sorry.google.com") {
        return Err(anyhow!("Google returned a sorry/CAPTCHA page"));
    }

    let body = response
        .into_string()
        .context("failed to read Google search response")?;
    if body.contains("captcha-form") || body.contains("/sorry/index") {
        return Err(anyhow!("Google returned a CAPTCHA page"));
    }

    Ok(SearchResponse {
        provider: ProviderKind::Google.as_str().to_string(),
        hits: parse_google_results(&body, query.offset),
        evidence: Vec::new(),
        executed_queries: vec![query.text.clone()],
    })
}
...
fn extract_pdf_sections_guided(
    ...
    let mut doc = pdf_extract::Document::load_mem(body).context("failed to parse PDF bytes")?;
    if doc.is_encrypted() {
        doc.decrypt("")
            .map_err(pdf_extract::OutputError::PdfError)
            .context("failed to decrypt PDF")?;
    }

    let page_numbers = doc.get_pages().into_keys().collect::<Vec<_>>();
    ...
    for page_num in candidate_pages {
        let cleaned = extract_single_pdf_page_text(&doc, page_num)?;
        if cleaned.is_empty() {
            continue;
        }
...
fn extract_single_pdf_page_text(doc: &pdf_extract::Document, page_num: u32) -> Result<String> {
    let mut page_text = String::new();
    let mut output = pdf_extract::PlainTextOutput::new(&mut page_text);
    pdf_extract::output_doc_page(doc, &mut output, page_num)
        .with_context(|| format!("failed to extract PDF page {}", page_num))?;
    Ok(clean_pdf_text_for_llm(&page_text))
}
...
fn parse_google_results(body: &str, absolute_offset: usize) -> Vec<SearchHit> {
    static RESULT_SELECTOR: OnceLock<Selector> = OnceLock::new();
    static TITLE_SELECTOR: OnceLock<Selector> = OnceLock::new();
    static SNIPPET_SELECTOR: OnceLock<Selector> = OnceLock::new();

    let document = Html::parse_document(body);
    let result_selector = RESULT_SELECTOR.get_or_init(|| {
        Selector::parse("a[data-ved]:not([class]), div.g a[href]").expect("valid selector")
    });
    let title_selector = TITLE_SELECTOR.get_or_init(|| {
        Selector::parse("h3, div[role='heading'], div[style]").expect("valid selector")
    });
    let snippet_selector = SNIPPET_SELECTOR.get_or_init(|| {
        Selector::parse("div.VwiC3b, div[data-sncf='1'], div.ilUpNd.H66NU.aSRlid, span")
    ...
    for node in document.select(result_selector) {
        ...
        let url = normalize_google_result_url(href);
        ...
        let title = node
            ...
            .next()
            .map(|value| normalize_ws(&value.text().collect::<Vec<_>>().join(" ")))
            .filter(|text| !text.is_empty())
            .unwrap_or_else(|| display_url(&url));
        let snippet = node
            .select(snippet_selector)
            .map(|value| normalize_ws(&value.text().collect::<Vec<_>>().join(" ")))
            .find(|text| text.len() >= 20)
            .unwrap_or_default();
...
fn build_agent(config: &SearchConfig) -> ureq::Agent {
...
fn normalize_google_result_url(raw: &str) -> String {
    if let Some(stripped) = raw.strip_prefix("/url?q=") {
        let first = stripped.split('&').next().unwrap_or(stripped);
        percent_decode_lossy(first)
    } else {
        raw.to_string()
    }
}
...
<html>
  <body>
    ...
    fn execute_search_uses_cached_search_when_external_web_access_is_disabled() {
        ...
        let warm = execute_search(&root, &config, &live_request, &query).expect("warm search");
        let cached =
            execute_search(&root, &config, &cached_request, &query).expect("cached search");

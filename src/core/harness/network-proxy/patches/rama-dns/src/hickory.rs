...
impl HickoryDnsBuilder {
    ...
    pub fn build(self) -> HickoryDns {
        self.try_build().unwrap_or_else(|error| {
            panic!("static Hickory DNS resolver configuration is invalid: {error}")
        })
    }

    fn try_build(self) -> Result<HickoryDns, OpaqueError> {
        ...
        resolver_builder
            .build()
            .context("build configured Hickory DNS resolver")
            .map(|resolver| HickoryDns(Arc::new(resolver)))
    }
}

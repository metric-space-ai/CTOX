...
use anyhow::Result;
use anyhow::anyhow;
use ctox_client::build_reqwest_client_with_custom_ca;
...
type StreamableHttpResponseClient = reqwest::Client;
...
impl ProcessGroupGuard {
    ...
    fn maybe_terminate_process_group(&self) {
        ...
        let should_escalate =
            match ctox_utils_pty::process_group::terminate_process_group(process_group_id) {
                Ok(exists) => exists,
                Err(error) => {
        ...
        if should_escalate {
            std::thread::spawn(move || {
                ...
                if let Err(error) =
                    ctox_utils_pty::process_group::kill_process_group(process_group_id)
                {
...
impl RmcpClient {
    ...
    fn is_session_expired_404(error: &ClientOperationError) -> bool {
        ...
        error
            ...
            .is_some_and(|error| matches!(error, StreamableHttpError::SessionExpired))
    }

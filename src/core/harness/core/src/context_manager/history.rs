...
use ctox_utils_cache::BlockingLruCache;
use ctox_utils_cache::sha1_digest;
...
impl ContextManager {
    ...
    fn normalize_history(&mut self, input_modalities: &[InputModality]) {
        // Do not rewrite earlier CTOX runtime snapshots here. Between
        // compactions the provider-visible transcript must remain append-only
        // so Responses `previous_response_id` chaining and prefix caches can
        // reuse the complete prior request. Compaction owns replacement
        // history and re-injects one canonical current runtime snapshot.
    ...
    ORIGINAL_IMAGE_ESTIMATE_CACHE
        .get_or_try_insert_with(key, || -> Result<Option<i64>, std::convert::Infallible> {
            let payload = match parse_base64_image_data_url(image_url) {
                ...
                None => {
                    tracing::trace!(
                        "skipping original-detail estimate for non-base64 image data URL"
                    );
                    return Ok(None);
                }
            };

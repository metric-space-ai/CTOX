mod registry;
use registry::{
    count_rows, list_targets, load_registered_target, open_db, register_script,
    register_source_module, resolve_db_path, resolve_registered_script_path, show_api, show_target,
    upsert_target, workspace_belongs_to_stale_release,
};
mod execute;
pub(crate) use execute::execute_scrape_with_outcome;
use execute::{
    acquire_target_run_lock, execute_registered_script, execute_scrape,
    is_preserved_runner_env_key, kill_runner_process_tree, probe_portal_health,
    run_lock_holder_is_alive, CommandExecution, ProbeResult, TargetRunLock,
};
mod semantic_enrichment;
pub(crate) use semantic_enrichment::service_semantic_search;
use semantic_enrichment::{
    apply_enrichment_updates, cosine_similarity, default_embedding_model,
    default_llm_enrichment_config, default_semantic_config_for_target, embed_texts,
    embed_texts_via_local_socket, enrichment_filter_paths, ensure_semantic_records,
    load_llm_enrichment_config, load_semantic_config, load_semantic_matches,
    maybe_run_llm_enrichment, rebuild_semantic_index, semantic_search, semantic_text_for_record,
    EnrichmentConfig, SemanticConfig,
};
mod outputs;
use outputs::{
...
mod reauth;
use reauth::{
    emit_reauthorization_handoff, protected_config_from_script, resolve_protected_capture_config,
    session_expiry_reauthorization, url_is_login_landing, valid_credential_reference,
    ProtectedCaptureConfig,
};
mod cli;
pub use cli::handle_scrape_command;
mod classify;
pub(crate) use classify::ScrapeRunStatus;
use classify::{classify_outcome, contains_transient_hint, Classification};

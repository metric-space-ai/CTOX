...
pub fn tool_descriptors() -> Vec<BusinessOsMcpToolDescriptor> {
    let mut tools = vec![
        ...
        write_tool(
            ...
            "Persist completed lead research via its contracted native business_command. Requires the signed research task session. The server binds module and research_command_id; no CLI, shell or SQLite access is needed. Check the returned status; failed writeback means the task is not complete. Exact shape (all keys lowercase, lists are JSON arrays, never strings): {\"record_id\": \"<lead id>\", \"payload\": {\"field_status\": {\"<field>\": {\"status\": \"verified|no_match|unsupported|action_required\", \"value\": <only for verified>, \"reason\": \"<why, for non-verified>\", \"sources\": [{\"source_id\": \"host\", \"url\": \"https://…\", \"quote\": \"verbatim\"}], \"person_key\": \"<for person_* fields>\"}}, \"result\": {\"fields\": {\"<verified field>\": {\"value\": <same as field_status>, \"sources\": [...]}}, \"person_records\": [{\"person_key\": \"…\", \"person_vorname\": \"…\", \"person_nachname\": \"…\", \"person_funktion\": \"…\", \"sources\": [...]}], \"evidence\": [{\"field_key\": \"…\", \"source_id\": \"…\", \"url\": \"…\", \"quote\": \"…\"}]}}}. Do not wrap the payload in \"item\", do not put field keys at the top level, and cover every requested field in field_status.",
        ...
        write_tool(
            ...
            object_schema(vec![required_string("url"), optional_string("state_dir")]),
        ),
        write_tool(
            "appsec_lab_create",
            ...
            object_schema(vec![optional_string("state_dir"), optional_string("out")]),
        ),
        write_tool(
            "appsec_lab_run",
        ...
        read_tool(
            ...
            object_schema(vec![optional_string("state_dir"), optional_boolean("sync")]),
        ),
        read_tool(
...
fn validate_person_research_record_binding(
    ...
    const MODULE_ID: &str = "outbound-lead-generation";
    ...
    let owning_modules = module_ids_for_collection(root, COLLECTION)?;
    anyhow::ensure!(
        owning_modules
            .iter()
    ...
    let record = store::pull_collection_record(root, COLLECTION, record_id)?.ok_or_else(|| {
        anyhow::Error::new(BusinessOsMcpError::not_found(
            BusinessOsMcpErrorCode::RecordNotFound,
...
mod tests {
    ...
    fn upsert_record_persists_app_data_for_admin_mcp_actor() -> anyhow::Result<()> {
        ...
        let conn = rusqlite::Connection::open(rxdb)?;
        let table: String = conn.query_row(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE '%customer_accounts%' LIMIT 1",
            [],
            |row| row.get(0),
        )?;
        let projected: String = conn.query_row(
            &format!("SELECT data FROM {table} WHERE id = ?1 AND deleted = 0"),
            rusqlite::params!["acct_mcp_1"],
            |row| row.get(0),
        )?;
    ...
    fn person_research_execute_is_idempotent_and_record_bound() -> anyhow::Result<()> {
        ...
        let (first, replay) = std::thread::scope(|scope| {
            ...
            (
                ...
                replay.join().expect("replay execute thread"),
            )
        });
        let first = first?;
        let replay = replay?;
        assert_eq!(first.command_id, replay.command_id);
        assert_eq!(replay.client_context["idempotency_key"], "research-1");
        assert_eq!(
            replay.client_context["writeback_contract"],
            "person_research/native"
        );
        ...
        assert_ne!(first.command_id, next.command_id);

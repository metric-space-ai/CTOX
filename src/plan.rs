...
pub fn emit_due_steps(root: &Path) -> Result<EmitDuePlansSummary> {
    ...
    for goal_id in goals {
        if let Some(emitted) = emit_next_step_tx(root, &conn, &goal_id)? {
...
fn list_goals(root: &Path) -> Result<Vec<PlannedGoalView>> {
    let conn = open_plan_db(root)?;
...
pub fn emit_next_step_for_goal(root: &Path, goal_id: &str) -> Result<Option<EmittedPlanStepView>> {
    let conn = open_plan_db(root)?;
    emit_next_step_tx(root, &conn, goal_id)
}

fn emit_next_step_tx(
    root: &Path,
    conn: &Connection,
    goal_id: &str,
    ...
    tx.execute(
        ...
        WHERE step_id = ?1
        "#,
        params![step.step_id, STEP_STATUS_QUEUED, message_key, now],
    )?;
    tx.execute(
        r#"
        ...
        WHERE goal_id = ?1
        "#,
        params![goal.goal_id, GOAL_STATUS_ACTIVE, now],
    )?;
    tx.commit()?;

...
impl ModelClientSession {
    ...
    #[allow(clippy::too_many_arguments)]
    #[instrument(
        name = "model_client.stream_responses_local_socket",
        level = "info",
        skip_all,
        fields(
            model = %model_info.slug,
            wire_api = %self.client.state.provider.wire_api,
            transport = "responses_local_socket",
            api.path = "responses_socket",
            turn.has_metadata_header = turn_metadata_header.is_some()
        )
    )]
    async fn stream_responses_local_socket(
        &self,
        socket_path: &str,
        local_request: LocalSocketResponsesRequest,
        model_info: &ModelInfo,
        session_telemetry: &SessionTelemetry,
        turn_metadata_header: Option<&str>,
    ) -> Result<ResponseStream> {
        let socket_path = socket_path.to_string();
        let request_dump_path = local_socket_request_dump_path(&socket_path);
        ...
        tokio::spawn(async move {
            let result: std::result::Result<(), ApiError> = async {
                ...
                if let Some(path) = request_dump_path.as_ref() {
                    ...
                    if let Ok(pretty_payload) = serde_json::to_vec_pretty(&local_request) {
                        let _ = std::fs::write(path, pretty_payload);
                    }
                }
                let payload = serde_json::to_vec(&local_request)
                    .map_err(|err| ApiError::Stream(err.to_string()))?;
                writer
                    .write_all(&payload)
                    .await
            ...
            WireApi::Responses => {
                #[cfg(unix)]
                {
                    let socket_transport_required =
                        self.client.state.provider.requires_socket_transport();
                    match self.client.state.provider.socket_path.as_deref() {
                        Some(socket_path) => {
                            let client_setup = self.client.current_client_setup().await?;
                            let request = self.build_responses_request(
                                &client_setup.api_provider,
                            ...
                            match build_local_socket_request(&request) {
                                ...
                                Err(err) if socket_transport_required => {
                                    return Err(CodexErr::Stream(
                                        format!(
                                            "CTOX-managed local runtime requires socket transport, but the request cannot be represented over the local socket: {err}"
                                        ),
                                        None,
                                    ));
                                }
                                Err(err) => {
                                    warn!(
                                        reason = %err,
                                        "falling back to HTTP responses transport because the local socket path cannot represent this request"
                                    );
                                }
                            }
                        }
                        None if socket_transport_required => {
                            return Err(CodexErr::Stream(
                                "CTOX-managed local runtime requires socket transport, but no socket path is configured".to_string(),
                                None,
                            ));
                        }
                        None => {}
...
struct LocalSocketResponsesRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    text: Option<codex_api::common::TextControls>,
}

fn build_local_socket_request(
    request: &ResponsesApiRequest,
) -> std::result::Result<LocalSocketResponsesRequest, String> {
    Ok(LocalSocketResponsesRequest {
        ...
        input: request
            ...
            .iter()
            .enumerate()
            .filter_map(|(index, item)| map_input_item_for_local_socket(item, index))
            .collect(),
        tools: normalize_tools_for_local_socket(&request.tools)?,
...
fn local_runtime_gpt_oss_max_output_tokens(
    ...
    let path = local_runtime_state_path(provider)?;
    let raw = std::fs::read(&path).ok()?;
    let snapshot: LocalRuntimeStateSnapshot = serde_json::from_slice(&raw).ok()?;
    let cap = snapshot
        .adapter_tuning
...
fn local_runtime_state_path(
    provider: &crate::model_provider_info::ModelProviderInfo,
) -> Option<std::path::PathBuf> {
    let socket_path = provider.socket_path.as_deref()?;
    let socket_path = std::path::Path::new(socket_path);
    let sockets_dir = socket_path.parent()?;
    ...
    runtime_dir
        .parent()
        .map(|root| root.join("runtime/inference_runtime.json"))
}

fn local_socket_request_dump_path(socket_path: &str) -> Option<std::path::PathBuf> {
    ...
    Some(runtime_dir.join("last_local_socket_request.json"))
}

fn normalize_tools_for_local_socket(tools: &[Value]) -> std::result::Result<Vec<Value>, String> {
    let mut normalized = Vec::new();
    for tool in tools {
        normalized.extend(normalize_tool_for_local_socket(tool)?);
    }
    Ok(normalized)
}

fn normalize_tool_for_local_socket(tool: &Value) -> std::result::Result<Vec<Value>, String> {
    let object = tool
        .as_object()
    ...
    match tool_type {
        "function" => Ok(vec![normalize_function_tool_for_local_socket(object)?]),
        "namespace" => {
            ...
            {
                flattened.extend(normalize_tool_for_local_socket(child)?);
            }
            Ok(flattened)
        }
        "custom" => {
...
fn normalize_function_tool_for_local_socket(
...
fn map_input_item_for_local_socket(item: &ResponseItem, index: usize) -> Option<Value> {
    match item {
        ResponseItem::Message { role, content, .. } => {
            let parts = content
                .iter()
                .filter_map(map_content_item_for_local_socket)
                .collect::<Vec<_>>();
            if parts.is_empty() {
                return None;
            }
            ...
            let text = render_message_content_for_local_socket(content);
        ...
        | ResponseItem::CustomToolCallOutput { call_id, output } => Some(json!({
            ...
            "call_id": call_id,
            "output": render_output_for_local_socket(output),
        })),
        ...
        ResponseItem::WebSearchCall { action, .. } => {
            let text = action
                .as_ref()
                .map(render_web_search_action_for_local_socket)
...
fn map_content_item_for_local_socket(item: &ContentItem) -> Option<Value> {
    match item {
...
fn render_message_content_for_local_socket(content: &[ContentItem]) -> String {
...
fn render_output_for_local_socket(
...
fn render_web_search_action_for_local_socket(
    ...
    match action {
        ...
        codex_protocol::models::WebSearchAction::Other => "Web search call".to_string(),
    }
}

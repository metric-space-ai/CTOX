...
fn test_local_socket_model_client(session_source: SessionSource) -> ModelClient {
    let provider = crate::model_provider_info::ModelProviderInfo {
        name: "cto-local".to_string(),
        base_url: None,
        socket_path: Some("/tmp/ctox.sock".to_string()),
...
#[test]
fn local_socket_request_omits_native_web_search_tools() {
    ...
    let normalized =
        super::build_local_socket_request(&request).expect("web_search should be omitted");
    assert!(normalized.tools.is_empty());
}

#[test]
fn local_socket_request_preserves_function_tools_including_spawn_agent() {
    let request = ResponsesApiRequest {
        model: "gpt-test".to_string(),
        instructions: "test instructions".to_string(),
        input: Vec::new(),
    ...
    let normalized =
        super::build_local_socket_request(&request).expect("spawn_agent should stay representable");
...
#[test]
fn local_socket_request_forces_parallel_tool_calls_on() {
    let request = ResponsesApiRequest {
        model: "gpt-test".to_string(),
        instructions: "test instructions".to_string(),
        input: Vec::new(),
    ...
    let normalized =
        super::build_local_socket_request(&request).expect("request should stay representable");
...
fn managed_local_responses_max_output_tokens_reads_runtime_state_override() {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("codex-client-runtime-state-{unique}"));
    std::fs::create_dir_all(root.join("runtime/sockets")).unwrap();
    std::fs::write(
        root.join("runtime/inference_runtime.json"),
...
fn managed_local_responses_max_output_tokens_defaults_to_realized_context_budget() {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("codex-client-runtime-budget-{unique}"));
    std::fs::create_dir_all(root.join("runtime/sockets")).unwrap();
    std::fs::write(
        root.join("runtime/inference_runtime.json"),
...
fn managed_local_gpt_oss_request_preserves_explicit_none_reasoning() {
    let client = test_local_socket_model_client(SessionSource::Cli);
...
#[test]
fn local_socket_request_collapses_non_user_image_messages_to_text() {
    let request = ResponsesApiRequest {
        model: "gpt-test".to_string(),
        instructions: "test instructions".to_string(),
...
#[test]
fn local_socket_request_collapses_non_user_multipart_messages_to_text() {
    let request = ResponsesApiRequest {
        model: "gpt-test".to_string(),
        instructions: "test instructions".to_string(),
        input: vec![ResponseItem::Message {

//! Proc macros for ergonomic tool definition in engine.rs
//!
//! This crate provides the `#[tool]` attribute macro for defining tools
//! that can be used with the engine.rs agentic loop.
//!
//! # Example
//!
//! ```ignore
//! use engine_macros::tool;
...
/// used with the engine.rs agentic loop. It generates:
///
...
fn generate_tool_impl(args: ToolArgs, input_fn: ItemFn) -> syn::Result<TokenStream2> {
    ...
    let output = if is_async {
        ...
        quote! {
            ...
            #fn_vis fn #tool_fn_name() -> engine::Tool {
            ...
            #fn_vis fn #callback_fn_name() -> std::sync::Arc<engine::AsyncToolCallback> {
                std::sync::Arc::new(|called: engine::CalledFunction| {
                    Box::pin(async move {
            ...
            #fn_vis fn #combined_fn_name() -> (engine::Tool, engine::ToolCallbackType) {
                (#tool_fn_name(), engine::ToolCallbackType::Async(#callback_fn_name()))
            }
        }
    } else {
        ...
        quote! {
            ...
            #fn_vis fn #callback_fn_name() -> std::sync::Arc<engine::ToolCallback> {
                std::sync::Arc::new(|called: &engine::CalledFunction| {
                    let args: #args_struct_name = serde_json::from_str(&called.arguments)
                        .map_err(|e| anyhow::anyhow!("Failed to parse tool arguments: {}", e))?;
            ...
            /// Returns both the Tool definition and callback as a tuple
            #fn_vis fn #combined_fn_name() -> (engine::Tool, engine::ToolCallbackType) {
                (#tool_fn_name(), engine::ToolCallbackType::Sync(#callback_fn_name()))
            }
        }
    };

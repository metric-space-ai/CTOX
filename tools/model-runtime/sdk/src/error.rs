//! Error types and conversions for the engine SDK.
...
pub enum Error {
    ...
    ModelError {
        ...
        partial_response: Option<Box<engine_core::ChatCompletionResponse>>,
    },
    ...
    /// Wraps the existing [`engine_core::MistralRsError`].
    #[error(transparent)]
    Management(#[from] engine_core::MistralRsError),
...
impl From<tokio::sync::mpsc::error::SendError<engine_core::Request>> for Error {
    /// Convert from a channel send error (mapped to [`Error::Channel`]).
    fn from(e: tokio::sync::mpsc::error::SendError<engine_core::Request>) -> Self {
        Error::Channel(e.to_string())
    }
}

impl From<Box<engine_core::ResponseErr>> for Error {
    /// Convert from a boxed engine response error.
    fn from(e: Box<engine_core::ResponseErr>) -> Self {
        match *e {
            engine_core::ResponseErr::InternalError(inner) => Error::Inference(inner),
            engine_core::ResponseErr::ValidationError(inner) => {
                Error::RequestValidation(inner.to_string())
            }
            engine_core::ResponseErr::ModelError(message, response) => Error::ModelError {
                message,
                partial_response: Some(Box::new(response)),
            },
            engine_core::ResponseErr::CompletionModelError(message, _) => Error::ModelError {
                message,

...
impl MultiModelBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<Model> {
        ...
        let mut runner_builder = engine_core::MistralRsBuilder::new(
            pipeline,
            scheduler_config,
        ...
        let engine = runner_builder.build().await;
...
pub(crate) fn maybe_initialize_logging(with_logging: bool) {
    if with_logging {
        engine_core::initialize_logging();
    }
}
...
pub(crate) async fn build_pipeline_from_text_loader(
    ...
    loader: Box<dyn engine_core::Loader>,
) -> anyhow::Result<(Arc<Mutex<dyn Pipeline>>, SchedulerConfig, AddModelConfig)> {
    use engine_core::*;
    ...
    let device_map_setting =
        builder
            .device_mapping
            .clone()
            .unwrap_or(engine_core::DeviceMapSetting::Auto(
                engine_core::AutoDeviceMapParams::default_text(),
            ));
...
pub async fn build_model_from_pipeline(
    pipeline: Arc<Mutex<dyn engine_core::Pipeline>>,

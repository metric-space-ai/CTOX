...
fn main() -> Result<(), String> {
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        // https://github.com/EricLBuehler/engine.rs/issues/286
        ...
        let build_dir = PathBuf::from(std::env::var("OUT_DIR").unwrap());
        // https://github.com/EricLBuehler/engine.rs/issues/588
        let out_file = if target.contains("msvc") {
            // Windows case
            build_dir.join("enginequant.lib")
        } else {
            build_dir.join("libenginequant.a")
        };
        builder
            .build_lib(out_file)
            .expect("Build engine quant lib failed!");
        println!("cargo:rustc-link-search={}", build_dir.display());
        println!("cargo:rustc-link-lib=enginequant");
    ...
    #[cfg(feature = "metal")]
    {
        ...
        // https://github.com/EricLBuehler/engine.rs/pull/1311#issuecomment-3001309885
        println!("cargo:rerun-if-env-changed=ENGINE_METAL_PRECOMPILE");
        let skip_precompile = env::var("ENGINE_METAL_PRECOMPILE")
            .map(|v| v == "0" || v.to_lowercase() == "false")
            .unwrap_or(false);

        if skip_precompile {
            println!(
                "cargo:warning=Skipping Metal kernel precompilation (ENGINE_METAL_PRECOMPILE=0)"
            );
            ...
            std::fs::write(out_dir.join("engine_quant.metallib"), []).unwrap();
            std::fs::write(out_dir.join("engine_quant_ios.metallib"), []).unwrap();
            std::fs::write(out_dir.join("engine_quant_tvos.metallib"), []).unwrap();
            return Ok(());
        }
        ...
        impl Platform {
            ...
            fn metal_std(&self) -> &str {
                ...
                // https://github.com/EricLBuehler/engine.rs/issues/1844
                //
        ...
        fn compile(platform: Platform) -> Result<(), String> {
            ...
            let lib_name = match platform {
                Platform::MacOS => "engine_quant.metallib",
                Platform::Ios => "engine_quant_ios.metallib",
                Platform::TvOS => "engine_quant_tvos.metallib",
            };

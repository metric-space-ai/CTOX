...
pub use gguf::{stack_gguf_experts, GgufMatMul};
...
pub fn create_isq_thread_pool(ty: Option<IsqType>) -> (rayon::ThreadPool, usize) {
    let is_distributed = std::env::var("ENGINE_MN_GLOBAL_WORLD_SIZE")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .is_some_and(|world_size| world_size > 1)
        || std::env::var("ENGINE_MN_LOCAL_WORLD_SIZE")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .is_some_and(|world_size| world_size > 1)
    ...
    let explicit_thread_limit = std::env::var("ENGINE_ISQ_CPU_THREADS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
    ...
    let num_threads = if let Some(num_threads) = explicit_thread_limit {
        num_threads
    } else if std::env::var("ENGINE_ISQ_SINGLETHREAD").is_ok() {
        1
    ...
    pub fn qmethod_matmul(&self, x: &Tensor, matmul: &dyn QuantMethod) -> Result<Tensor> {
        matmul.forward(x).map_err(|err| {
            ...
            err.with_path(format!(
                "qmethod_matmul(name={}, input_shape={:?}, input_rank={}, input_dtype={:?}, input_device={:?}, weight_dtype={:?}, weight_device={:?})",
                matmul.name(),
                x.dims(),
                x.rank(),
                x.dtype(),
...
pub trait QuantMethod: Send + Sync + Debug + QuantizedSerde {
    ...
    /// Resolve any deferred quantization work before the layer is first used.
    fn finalize_immediate_isq(&self) -> Result<()> {
        Ok(())
    }

...
impl QuantMethod for RowParallelLayer {
    ...
    fn forward(&self, a: &Tensor) -> Result<Tensor> {
        ...
        let mut xs = self.weight.forward(a).map_err(|err| {
            err.with_path(format!(
                "row_parallel.weight_forward(input_shape={:?}, input_device={:?}, weight_name={})",
                a.shape().dims(),
                a.device().location(),
                self.weight.name(),
            ))
        })?;
        let xs_contig = xs.contiguous().map_err(|err| {
            err.with_path(format!(
                "row_parallel.contiguous(output_shape={:?}, output_device={:?}, weight_name={})",
                xs.shape().dims(),
                xs.device().location(),
                self.weight.name(),
            ))
        })?;
        if std::env::var("ENGINE_DEBUG_TP_SHAPES").ok().as_deref() == Some("1") {
            tracing::info!(
                "row_parallel.pre_all_reduce shape={:?} dtype={:?} device={:?} weight_name={}",
                xs_contig.shape().dims(),
                xs_contig.dtype(),
                xs_contig.device().location(),
                self.weight.name(),
            );
        }
        xs = self.all_reduce.sum_all_reduce(&xs_contig).map_err(|err| {
            err.with_path(format!(
                "row_parallel.all_reduce(output_shape={:?}, output_device={:?}, weight_name={})",
                xs_contig.shape().dims(),
                xs_contig.device().location(),
                self.weight.name(),
            ))
...
impl PackedExperts {
    ...
    pub fn new(
        ...
        let (gate_proj, up_proj, down_proj) = if let Some(quant_conf) = &config {
            ...
            let shard_gate_alt = Shard::Offset {
                dim: 1,
                offset: gate_up_start,
                len: gate_up_block_size,
            };
            ...
            let shard_down_alt = Shard::Simple {
                dim: 2,
                rank: comm.rank(),
                world_size: comm.world_size(),
            };

            let target_vb_gate_proj = vb.pp("gate_proj");
            let target_vb_up_proj = vb.pp("up_proj");
            let target_vb_down_proj = vb.pp("down_proj");
            ...
            let vb_down_proj = vb_gate_up_proj.clone();
            ...
            let gate_proj = vb_gate_up_proj
                ...
                .and_then(|primary| {
                    let (_, gate_out, gate_in) = primary.dims3()?;
                    if gate_out == gate_up_block_size && gate_in == hidden_size {
                        Ok(primary)
                    } else {
                        candle_core::bail!(
                            "stacked gate_up_proj shard uses alternate layout: expected [{num_local_experts}, {gate_up_block_size}, {hidden_size}], got {:?}",
                            primary.dims()
                        );
                    }
                })
                .or_else(|_| {
                    vb_gate_up_proj.get_with_hints(
            ...
            let up_proj = vb_gate_up_proj
                ...
                .and_then(|t| t.transpose(1, 2)?.contiguous())
                .and_then(|primary| {
                    let (_, up_out, up_in) = primary.dims3()?;
                    if up_out == gate_up_block_size && up_in == hidden_size {
                        Ok(primary)
                    } else {
                        candle_core::bail!(
                            "stacked gate_up_proj shard uses alternate layout: expected [{num_local_experts}, {gate_up_block_size}, {hidden_size}], got {:?}",
            ...
            let down_proj = vb_down_proj
                ...
                .and_then(|t| t.transpose(1, 2)?.contiguous())
                .and_then(|primary| {
                    let (_, down_out, down_in) = primary.dims3()?;
                    if down_out == intermediate_size && down_in == hidden_size {
                        Ok(primary)
                    } else {
                        candle_core::bail!(
                            "stacked down_proj shard uses alternate layout: expected [{num_local_experts}, {intermediate_size}, {hidden_size}], got {:?}",
                            primary.dims()
                        );
                    }
                })
                .or_else(|_| {
                    vb_down_proj.get_with_hints(
            ...
            {
                gate_proj = gate_proj.squeeze(0)?.contiguous()?;
                up_proj = up_proj.squeeze(0)?.contiguous()?;
                down_proj = down_proj.squeeze(0)?.contiguous()?;
                ...
                // Read raw stacked experts on CPU during immediate ISQ, but quantize
                // onto the original layer device so first decode does not relocate
                // each active expert's qweights from CPU to CUDA on demand.
                gate_proj = apply_immediate_isq(gate_proj, target_vb_gate_proj.clone())?;
                let mut up_proj: Arc<dyn QuantMethod> =
                    Arc::new(<UnquantLinear as QuantMethod>::new(
                        QuantMethodConfig::Unquantized(Linear::new(up_proj, None)),
                    )?);
                up_proj = apply_immediate_isq(up_proj, target_vb_up_proj.clone())?;
                let mut down_proj: Arc<dyn QuantMethod> =
                    Arc::new(<UnquantLinear as QuantMethod>::new(
                        QuantMethodConfig::Unquantized(Linear::new(down_proj, None)),
                    )?);
                down_proj = apply_immediate_isq(down_proj, target_vb_down_proj.clone())?;
                gs.push(gate_proj);
                us.push(up_proj);
                ds.push(down_proj);
            }
...
impl FusedExperts {
    pub fn new(
        ...
        let is_direct_experts_root = vb.contains_tensor("gate_up_proj")
        ...
        let experts_vb = if is_direct_experts_root {
            vb.clone()
        } else {
            vb.pp("experts")
        };
            ...
            && matches!(&quantization_config, Some(QuantizedConfig::Fp8 { .. }))
        {
            ...
            if has_fp8_scales {
                ...
                let target_device = target_device
                    .cloned()
                    .unwrap_or_else(|| gate_proj.device().clone());
                let (gate_proj, up_proj, down_proj) = if crate::get_immediate_isq().is_some() {
                    let load_device = if target_device.is_cpu() {
                        Device::Cpu
                    } else {
                        Device::Cpu
                    };
                    (
                        gate_proj.to_device(&load_device)?,
                        up_proj.to_device(&load_device)?,
                        down_proj.to_device(&load_device)?,
                    )
                } else {
            ...
            && matches!(&quantization_config, Some(QuantizedConfig::MXFP4 {}))
        {
            ...
            let quant_target_vb = target_device
                .map(|device| expert0_vb.clone().set_device(device.clone()))
                .unwrap_or_else(|| expert0_vb.clone());
            gate_proj = apply_immediate_isq(gate_proj, quant_target_vb.pp("gate_proj"))?;
            up_proj = apply_immediate_isq(up_proj, quant_target_vb.pp("up_proj"))?;
            down_proj = apply_immediate_isq(down_proj, quant_target_vb.pp("down_proj"))?;

...
pub fn get_global_tp_size_from_devices() -> Result<usize> {
    ...
    #[cfg(all(feature = "cuda", feature = "nccl"))]
    {
        ...
        if let Ok(x) = std::env::var("ENGINE_MN_LOCAL_WORLD_SIZE") {
            use std::str::FromStr;
            Ok(usize::from_str(&x).expect("Not a number for ENGINE_MN_LOCAL_WORLD_SIZE!"))
        } else {
...
pub fn use_nccl() -> bool {
    (std::env::var("ENGINE_NO_NCCL").is_err()
        || std::env::var("ENGINE_NO_NCCL").is_ok_and(|x| x != "1"))
...
mod nccl {
    ...
    impl NcclComm {
        ...
        pub fn with_inner<T>(&self, f: impl FnOnce(&cudarc::nccl::Comm) -> Result<T>) -> Result<T> {
            let comm = self.comm.lock().map_err(|err| {
                candle_core::Error::msg(format!("Failed to lock NCCL communicator: {err}"))
            })?;
            comm.context()
                .bind_to_thread()
    ...
    // NCCL communicators are not thread-safe for concurrent use. We only expose them behind a
    // process-local mutex and never hand out mutable access without holding that lock.
...
mod nccl_ops {
    ...
    use candle_core::{
        ...
        cuda::cudarc,
        CpuStorage, CustomOp1, DType, Layout, Result, Shape, Tensor,
    };
                ...
                super::Comm::Nccl(nccl_comm) => {
                    ...
                    let dst = match s.dtype() {
                        DType::BF16 => {
                            ...
                            let mut dst = unsafe { cuda_dev.alloc::<bf16>(elem_count) }?;
                            nccl_comm.with_inner(|comm| {
                                comm.all_reduce(s, &mut dst, &ReduceOp::Sum)
                                    .map_err(|e| candle_core::Error::debug(e.0))
                            })?;
                            candle_core::CudaStorage::wrap_cuda_slice(dst, cuda_dev.clone())
                        }
                        DType::F16 => {
                            ...
                            let mut dst = unsafe { cuda_dev.alloc::<f16>(elem_count) }?;
                            nccl_comm.with_inner(|comm| {
                                comm.all_reduce(s, &mut dst, &ReduceOp::Sum)
                                    .map_err(|e| candle_core::Error::debug(e.0))
                            })?;
                        ...
                        DType::F32 => {
                            ...
                            let mut dst = unsafe { cuda_dev.alloc::<f32>(elem_count) }?;
                            nccl_comm.with_inner(|comm| {
                                comm.all_reduce(s, &mut dst, &ReduceOp::Sum)
                                    .map_err(|e| candle_core::Error::debug(e.0))
                            })?;

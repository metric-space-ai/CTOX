...
fn main() -> Result<()> {
    ...
    println!("cargo:rerun-if-changed=src/cuda/turbo_wht.cu");
    ...
    let out_file = if target.contains("msvc") {
        // Windows case
        build_dir.join("enginepagedattention.lib")
    } else {
        build_dir.join("libenginepagedattention.a")
    };
    builder
        .build_lib(out_file)
    ...
    println!("cargo:rustc-link-lib=enginepagedattention");
...
fn main() -> Result<(), String> {
    ...
    if skip_precompile {
        println!("cargo:warning=Skipping Metal kernel precompilation (ENGINE_METAL_PRECOMPILE=0)");
        ...
        std::fs::write(out_dir.join("engine_paged_attention.metallib"), []).unwrap();
        std::fs::write(out_dir.join("engine_paged_attention_ios.metallib"), []).unwrap();
        std::fs::write(out_dir.join("engine_paged_attention_tvos.metallib"), []).unwrap();
        return Ok(());
    }
    ...
    fn compile(platform: Platform) -> Result<(), String> {
        ...
        let lib_name = match platform {
            Platform::MacOS => "engine_paged_attention.metallib",
            Platform::Ios => "engine_paged_attention_ios.metallib",
            Platform::TvOS => "engine_paged_attention_tvos.metallib",
        };

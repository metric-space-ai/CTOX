...
#[cfg(target_os = "macos")]
const KERNELS: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/engine_paged_attention.metallib"));
#[cfg(target_os = "ios")]
const KERNELS: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/engine_paged_attention_ios.metallib"
));
#[cfg(target_os = "tvos")]
const KERNELS: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/engine_paged_attention_tvos.metallib"
));
    ...
    pub fn load_library(&self, device: &Device) -> Result<Library, MetalKernelError> {
        if let Some(lib) = LIBRARY.get() {
            ...
            let lib = if !KERNELS.is_empty() {
                ...
                // https://github.com/EricLBuehler/engine.rs/issues/1897
                ...
                // (e.g., when ENGINE_METAL_PRECOMPILE=0)
...
pub fn call_copy_blocks(
    ...
    let name = match ty {
        ...
        DType::U8 => "copy_blocks_uchar",
        other => {
            return Err(MetalKernelError::DTypeMismatch {
                expected: vec![DType::F32, DType::F16, DType::BF16, DType::U8],
                got: other,
            })
        }
    };

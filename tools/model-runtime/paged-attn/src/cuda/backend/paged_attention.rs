...
impl PagedAttention {
    ...
    >(
        ...
        if cache_dtype == 4 {
            if vc_rank != 5 {
                candle::bail!(
                    "paged-attention expects turboquant `value_cache` tensor to be of rank 5 \
                    (value_cache: {vc_l:?})"
                )
            }
        } else if vc_rank != 4 {
        ...
        if cache_dtype == 4 {
            let (v_num_blocks, v_num_kv_heads, value_payload_bytes, v_block_size, value_x) =
                vc_l.shape().dims5()?;
            if x != 1 || value_x != 1 {
                candle::bail!(
                    "turboquant paged-attention expects x=1 cache layout, got key x={x}, value x={value_x}"
                )
            }
            if num_blocks != v_num_blocks
                || num_kv_heads != v_num_kv_heads
                || block_size != v_block_size
                || head_size_kc != value_payload_bytes
            {
                candle::bail!(
                    "shape mismatch turboquant key_cache {:?} and value_cache {:?}",
                    kc_l.shape(),
                    vc_l.shape()
                )
            }

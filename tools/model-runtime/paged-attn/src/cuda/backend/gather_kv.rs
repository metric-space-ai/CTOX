...
pub fn gather_kv_cache(
    ...
    // Extract dimensions from cache shapes. TurboQuant caches are stored as
    // U8 payloads with x=1 and payload bytes in the third dimension.
    let (num_kv_heads, block_size, x, head_size) = if cache_dtype == DType::U8 {
        let (_, num_kv_heads, key_payload_bytes, block_size, x) = key_cache.dims5()?;
        let (_, value_heads, value_payload_bytes, value_block_size, value_x) =
            value_cache.dims5()?;
        if x != 1 || value_x != 1 {
            candle_core::bail!(
                "turboquant gather expects x=1 cache layout, got key x={x}, value x={value_x}"
            );
        }
        if value_heads != num_kv_heads || value_block_size != block_size {
            candle_core::bail!(
                "turboquant gather expects matching cache geometry, got key heads/block_size=({num_kv_heads},{block_size}) and value=({value_heads},{value_block_size})"
            );
        }
        if key_payload_bytes != value_payload_bytes {
            candle_core::bail!(
                "turboquant gather expects matching key/value payload bytes, got {key_payload_bytes} and {value_payload_bytes}"
            );
        }
        if key_payload_bytes % TURBO3_BLOCK_BYTES != 0 {
            candle_core::bail!(
                "turboquant3 gather expects payload bytes divisible by {TURBO3_BLOCK_BYTES}, got {key_payload_bytes}"
            );
        }
        let head_size = (key_payload_bytes / TURBO3_BLOCK_BYTES) * TURBO3_BLOCK_SIZE;
        (num_kv_heads, block_size, 1usize, head_size)
    } else {
        ...
        (num_kv_heads, block_size, x, head_size)
    };
    ...
    let cache_dtype_code: u32 = match cache_dtype {
        ...
        DType::U8 => 4,
        other => candle_core::bail!(
            "gather_kv_cache only supports f16, bf16, f32, f8e4m3, u8 cache (got {other:?})"
        ),
    };

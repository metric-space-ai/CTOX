//! CLI argument definitions for ctox-engine-cli
//!
...
use engine_core::TokenSource;
use serde::Deserialize;
use std::path::PathBuf;

/// Fast LLM inference engine
#[derive(Parser)]
#[command(name = "ctox-engine")]
...
pub struct DefaultModelOptions {
    ...
    pub arch: Option<engine_core::NormalLoaderType>,

    /// Model data type
    #[arg(long, default_value = "auto", value_parser = parse_dtype)]
    pub dtype: engine_core::ModelDType,
...
fn parse_arch(s: &str) -> Result<engine_core::NormalLoaderType, String> {
    s.parse()
}

fn parse_dtype(s: &str) -> Result<engine_core::ModelDType, String> {
    s.parse()
}
...
impl From<TuneProfileArg> for engine_core::TuneProfile {
    fn from(value: TuneProfileArg) -> Self {
        match value {
            TuneProfileArg::Quality => engine_core::TuneProfile::Quality,
            TuneProfileArg::Balanced => engine_core::TuneProfile::Balanced,
            TuneProfileArg::Fast => engine_core::TuneProfile::Fast,
        }
    }
}

impl From<SearchEmbeddingModelArg> for engine_core::SearchEmbeddingModel {
    fn from(value: SearchEmbeddingModelArg) -> Self {
        match value {
            SearchEmbeddingModelArg::EmbeddingGemma => {
                engine_core::SearchEmbeddingModel::EmbeddingGemma300M
            }
        }
    }
}

...
use axum::Router;
use engine::{Model, SearchEmbeddingModel};
use engine_core::{MistralRs, ModelCategory};
...
fn build_model_list(engine: &Arc<MistralRs>) -> IndexMap<String, UiModelInfo> {
    let mut models = IndexMap::new();
    if let Ok(list) = engine.list_models() {
        for model_id in list {
            if let Ok(category) = engine.get_model_category(Some(&model_id)) {
...
pub async fn build_ui_router(
    ...
    let model_wrapper = Model::new(engine.clone());

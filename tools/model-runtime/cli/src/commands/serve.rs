...
use crate::args::{
    ...
    ModelType, QuantizationOptions, RuntimeOptions, ServerOptions, ServerTransport,
};

/// Run the local socket server with the specified model.
pub async fn run_server(
    ...
    info!("Starting engine server build.");
    let engine = builder.build().await?;
    info!("Finished engine server build.");
    #[cfg(unix)]
...
fn detect_speech_arch(model_id: &str) -> Result<SpeechLoaderType> {
    ...
    if lowered.contains("voxtral-4b-tts") || lowered.contains("voxtral_tts") {
        return Ok(SpeechLoaderType::VoxtralTts);
    }
    ...
    anyhow::bail!(
        "unable to determine speech architecture for `{model_id}`; supported native speech loaders are currently `voxtral_tts`, `qwen3_tts` and `dia`"
    )
}
    ...
    match model_type {
        ModelType::Auto {
                ...
                ModelFormat::Gguf | ModelFormat::Ggml => {
                    ...
                    if format.quantized_file.is_none() {
                        ...
                        anyhow::bail!(
                            ...
                            Usage: engine run auto -m <hf model/local dir> --format {fmt} -f <filename>\n\n\
            ...
            Ok(ModelSelected::Run {
                ...
                write_uqff: quantization.write_uqff.clone(),
...
pub(crate) fn extract_isq_setting(model_type: &ModelType) -> Option<String> {
    match model_type {
        ...
        ModelType::Speech { quantization, .. } => quantization.in_situ_quant.clone(),

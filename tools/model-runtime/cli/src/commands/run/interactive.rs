...
//!
//! Ported from engine-server/src/interactive_mode.rs
...
pub async fn interactive_mode(
    ...
    match engine.get_model_category(None) {
        Ok(ModelCategory::Text) => text_interactive_mode(engine, do_search, enable_thinking).await,
        Ok(ModelCategory::Vision { .. }) => {
            vision_interactive_mode(engine, do_search, enable_thinking).await
        }
        Ok(ModelCategory::Diffusion) => diffusion_interactive_mode(engine, do_search).await,
        Ok(ModelCategory::Audio) => {
            audio_interactive_mode(engine, do_search, enable_thinking).await
        }
        Ok(ModelCategory::Speech) => speech_interactive_mode(engine, do_search).await,
...
async fn text_interactive_mode(
    ...
    let sender = engine.get_sender(None).unwrap();
    ...
    'outer: loop {
        ...
        *CTRLC_HANDLER.lock().unwrap() = &terminate_handler;
        ...
        if let Some(last_usage) = last_usage {
            ...
            if let Ok(logger) = engine.get_logger(None) {
...
async fn vision_interactive_mode(
    ...
    let config = engine.config(None).unwrap();
...
async fn diffusion_interactive_mode(engine: Arc<MistralRs>, do_search: bool) {
    let sender = engine.get_sender(None).unwrap();
...
async fn speech_interactive_mode(engine: Arc<MistralRs>, do_search: bool) {
    let sender = engine.get_sender(None).unwrap();

...
pub async fn run_bench(
    ...
    if warmup > 0 {
        ...
        for _ in 0..warmup {
            let _ = run_single_bench(&engine, 32, 16).await;
        }
    ...
    for i in 0..iterations {
        ...
        if prompt_len > 0 {
            ...
            run_single_bench(&engine, prompt_len, 1).await?;
        ...
        if gen_len > 0 {
            let start = Instant::now();
            run_single_bench(&engine, 4, gen_len).await?;
            let elapsed = start.elapsed();
...
async fn run_single_bench(
    engine: &Arc<engine_core::MistralRs>,
    prompt_tokens: usize,
    gen_tokens: usize,
) -> Result<()> {

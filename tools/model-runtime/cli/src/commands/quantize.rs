...
use engine_core::{expand_isq_value, initialize_logging, IsqType, ModelSelected};
use engine_server_core::engine_for_server_builder::{defaults, MistralRsForServerBuilder};
...
pub async fn run_quantize(model_type: QuantizeModelType, global: GlobalOptions) -> Result<()> {
    ...
    for (i, isq) in expanded_isq.iter().enumerate() {
        ...
        let _engine = MistralRsForServerBuilder::new()
...
Run with `ctox-engine`. UQFF format notes still follow the upstream engine.rs documentation.
...
"#
    ...
    for (prefix, paths) in &groups {
        ...
        output += &format!(
            "|{quant_name}|`engine run -m {repo_display}{model_type_flag} --from-uqff {first_file}`|\n"
        );
    }

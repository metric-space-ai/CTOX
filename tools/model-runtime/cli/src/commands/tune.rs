...
use engine_core::{auto_tune, AutoTuneRequest, FitStatus, ModelSelected, QualityTier};
...
pub async fn run_tune(
    ...
    let requested_isq = extract_isq_setting(&model_type)
        .as_deref()
        .map(|s| {
            engine_core::parse_isq_value(s, None)
    ...
    println!("-------------------");
    if let Some(ref path) = emit_config {
        println!("  engine from-config --file {}", path.display());
        println!();
...
fn emit_toml_config(
    ...
    result: &engine_core::AutoTuneResult,
) -> Result<String> {

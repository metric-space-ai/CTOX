...
use candle_nn::{
    Conv1d, Conv1dConfig, ConvTranspose1d, ConvTranspose1dConfig, Embedding, LayerNorm,
};
use engine_quant::{
...
use crate::{
    layers::{self, conv1d, layer_norm, repeat_kv, Activation, RmsNorm, RotaryEmbedding},
...
impl CausalConv1d {
    ...
    fn extra_padding(&self, len: usize) -> usize {
        ...
        let ideal_length = ((n_frames.ceil() - 1.0) * self.stride as f64
            + (self.effective_kernel - padding) as f64) as usize;
...
impl Module for CausalConv1d {
    fn forward(&self, xs: &Tensor) -> candle_core::Result<Tensor> {
        let padding = self.effective_kernel.saturating_sub(self.stride);
        let padded =
            xs.pad_with_zeros(D::Minus1, padding, self.extra_padding(xs.dim(D::Minus1)?))?;
    ...
    fn forward(&self, xs: &Tensor) -> candle_core::Result<Tensor> {
        ...
        if self.right_pad > 0 {
            ys.narrow(
                D::Minus1,
                0,
                ys.dim(D::Minus1)?.saturating_sub(self.right_pad),
            )
        } else {
            Ok(ys)
        }
...
impl Module for ConvNeXtBlock {
    fn forward(&self, xs: &Tensor) -> candle_core::Result<Tensor> {
        ...
        let xs = self
            .pwconv2
            .forward(&self.act.forward(&self.pwconv1.forward(&xs)?)?)?;
        let xs = self
            .gamma
            .unsqueeze(0)?
            .broadcast_mul(&xs)?
...
impl DecoderSelfAttention {
    ...
    fn forward(
        ...
        let v = v.contiguous()?;
        let attn =
            (q.matmul(&k_t)? / (self.head_dim as f64).sqrt())?.broadcast_add(attention_mask)?;
        let attn = candle_nn::ops::softmax(&attn, D::Minus1)?.contiguous()?;
        let attn = attn.matmul(&v)?.transpose(1, 2)?.reshape((
            batch_size,
            seq_len,
...
impl VectorQuantization {
    fn new(
        ...
        Ok(Self {
            ...
            codebook: EuclideanCodebook::new(
                codebook_dim,
                codebook_size,
                epsilon,
                vb.pp("_codebook"),
    ...
    fn new(
        ...
        Ok(Self {
            ...
            residuals: [(1usize, 2usize), (3, 3), (9, 4)]
                .into_iter()
                .map(|(dilation, slot)| {
                    DecoderResidualUnit::new(out_dim, dilation, vb.pp("block").pp(slot))
                })
                .collect::<Result<Vec<_>>>()?,
        })
    }
...
impl Qwen3TtsTokenizerDecoder {
    ...
    fn forward_chunk(&self, codes: &Tensor) -> candle_core::Result<Tensor> {
        ...
        self.decoder_post
            .forward(&self.decoder_post_snake.forward(&hidden)?)?
            .clamp(-1.0, 1.0)
    }

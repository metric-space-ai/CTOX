...
/// Parsed assistant output recovered directly from Harmony-formatted text.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct HarmonyTextParse {
    pub analysis: String,
    pub commentary: String,
    pub final_content: String,
    pub tool_calls: Vec<HarmonyToolCall>,
    pub completed_final: bool,
    pub completed_tool_call: bool,
}

impl HarmonyTextParse {
    pub fn reasoning_content(&self) -> Option<String> {
    ...
    pub fn final_content(&self) -> Option<String> {
        if self.final_content.is_empty() {
            ...
            Some(self.final_content.clone())
        }
    }

    pub fn has_completed_final(&self) -> bool {
        self.completed_final
    }

    pub fn has_completed_tool_call(&self) -> bool {
        self.completed_tool_call
    }
}
...
pub struct HarmonyContext {
    ...
    // Track completed parser messages already incorporated into local state
    completed_messages_len: usize,
}
...
impl HarmonyContext {
    ...
    pub fn process_token(&mut self, token_id: u32) -> HarmonyDelta {
        ...
        let delta = self.extract_delta();
        self.maybe_log_token_event(token_id, &delta);
        delta
    }
    ...
    fn extract_delta(&mut self) -> HarmonyDelta {
        ...
        self.capture_completed_messages();
    ...
    fn capture_completed_messages(&mut self) {
        let new_messages = self
            .parser
            .messages()
            .get(self.completed_messages_len..)
        ...
        for message in &new_messages {
            ...
            if !is_tool_recipient(recipient) {
                continue;
            }

            self.tool_calls.push(HarmonyToolCall {
                id: format!("call_{}", Uuid::new_v4()),
                name: extract_tool_name(recipient),
                arguments: harmony_message_text(message),
            });

            if self
                .current_tool_call
                .as_ref()
                .is_some_and(|(current_recipient, _)| current_recipient == recipient)
            {
        ...
        self.completed_messages_len += new_messages.len();
        self.last_analysis_len = 0;
        self.last_commentary_len = 0;
        self.last_final_len = 0;
    }
    ...
    /// Check if any Harmony tool call has been fully completed.
    ...
    pub fn debug_state(&self) -> String {
        let current_tool_call = self
            .current_tool_call
            .as_ref()
            .map(|(recipient, args)| format!("{recipient}:{}b", args.len()))
            .unwrap_or_else(|| "none".to_string());
        format!(
            "channel={:?} recipient={:?} analysis_len={} commentary_len={} final_len={} sent_reasoning_len={} sent_final_len={} tool_calls={} current_tool_call={}",
            self.channel,
            self.parser.current_recipient(),
            self.accumulated.analysis.len(),
            self.accumulated.commentary.len(),
            self.accumulated.final_content.len(),
            self.sent_reasoning_len,
            self.sent_final_len,
            self.tool_calls.len(),
            current_tool_call,
        )
    ...
    fn maybe_log_token_event(&self, token_id: u32, delta: &HarmonyDelta) {
        ...
        let _ = writeln!(
            f,
            "Harmony token event: token_id={} delta={{channel={:?},analysis={:?},commentary={:?},final={:?}}} state={}",
            token_id,
            delta.current_channel,
            delta.analysis_delta,
...
fn is_tool_recipient(recipient: &str) -> bool {
    recipient.starts_with("functions.")
        || recipient.starts_with("browser.")
        || recipient == "python"
}

fn harmony_message_text(message: &openai_harmony::chat::Message) -> String {
    message
        .content
        .iter()
        .filter_map(|content| match content {
            openai_harmony::chat::Content::Text(text) => Some(text.text.as_str()),
            _ => None,
        })
        .collect::<String>()
}

static HARMONY_ASSISTANT_SEGMENT_RE: OnceLock<Regex> = OnceLock::new();

fn harmony_assistant_segment_regex() -> &'static Regex {
    HARMONY_ASSISTANT_SEGMENT_RE.get_or_init(|| {
        Regex::new(
            r#"(?s)<\|start\|>assistant(?:\s+to=(?P<to_before>[^\s<]+))?<\|channel\|>(?P<channel>analysis|commentary|final)(?:\s+to=(?P<to_after>[^\s<]+))?(?:\s*<\|constrain\|>[^\s<]+)?\s*<\|message\|>(?P<body>.*?)(?P<terminator><\|call\|>|<\|return\|>|<\|end\|>)"#,
        )
        .expect("valid Harmony assistant segment regex")
    })
}

/// Recover assistant-facing Harmony structure directly from rendered text.
///
/// This is a fallback for runtime paths where token IDs no longer line up with
/// the official Harmony encoding even though the detokenized text still carries
/// the correct `<|start|>...` markers.
pub fn parse_harmony_response_text(raw_text: &str) -> HarmonyTextParse {
    let mut parsed = HarmonyTextParse::default();

    for captures in harmony_assistant_segment_regex().captures_iter(raw_text) {
        let recipient = captures
            .name("to_before")
            .or_else(|| captures.name("to_after"))
            .map(|m| m.as_str().trim().to_string());
        let Some(channel) = captures
            .name("channel")
            .and_then(|m| HarmonyChannel::parse(m.as_str()))
        else {
            continue;
        };
        let body = captures
        ...
        if let Some(recipient) = recipient {
            if is_tool_recipient(&recipient) && matches!(channel, HarmonyChannel::Commentary) {
                parsed.tool_calls.push(HarmonyToolCall {
                    id: format!("call_{}", Uuid::new_v4()),
                    name: extract_tool_name(&recipient),
                    arguments: body.trim().to_string(),
                });
                parsed.completed_tool_call =
                    parsed.completed_tool_call || matches!(terminator, "<|call|>" | "<|end|>");
                continue;
            }
        }

        match channel {
            HarmonyChannel::Analysis => parsed.analysis.push_str(body),
            HarmonyChannel::Commentary => parsed.commentary.push_str(body),
            HarmonyChannel::Final => {
                parsed.final_content.push_str(body);
                parsed.completed_final =
                    parsed.completed_final || matches!(terminator, "<|return|>" | "<|end|>");
            }
        }
    }
...
/// Encode a Harmony prompt while preserving Harmony control tokens as special
/// tokens instead of ordinary text fragments.
pub fn encode_harmony_prompt_tokens(prompt: &str) -> Vec<u32> {
...
mod tests {
    ...
    #[test]
    fn test_harmony_context_completes_tool_call_on_call_token() {
        prewarm_harmony_encoding();
        let encoding = load_harmony_encoding(HarmonyEncodingName::HarmonyGptOss).unwrap();
        let tokens = encoding.tokenizer().encode_with_special_tokens(
            "<|start|>assistant<|channel|>commentary to=functions.exec_command<|constrain|>json<|message|>{\"cmd\":\"pwd\"}<|call|>",
        );

        let mut ctx = HarmonyContext::new().unwrap();
        for token in tokens {
            ctx.process_token(token);
        }

        assert!(ctx.has_completed_tool_calls());
        ctx.process_eos();

        let calls = ctx.finalize_tool_calls();
    ...
    #[test]
    fn test_parse_harmony_response_text_recovers_official_tool_call_shape() {
        let parsed = parse_harmony_response_text(
            "<|start|>assistant<|channel|>commentary to=functions.exec_command<|constrain|>json<|message|>{\"cmd\":\"pwd\"}<|call|>",
        );

        assert!(parsed.has_completed_tool_call());
        assert_eq!(parsed.tool_calls.len(), 1);
        assert_eq!(parsed.tool_calls[0].name, "exec_command");
        assert_eq!(parsed.tool_calls[0].arguments, "{\"cmd\":\"pwd\"}");
    }

    #[test]
    fn test_parse_harmony_response_text_keeps_legacy_tool_call_shape_compatible() {
        let parsed = parse_harmony_response_text(
    ...
    #[test]
    fn test_parse_harmony_response_text_recovers_reasoning_and_final() {
        let parsed = parse_harmony_response_text(
            "<|start|>assistant<|channel|>analysis<|message|>Need to answer carefully.<|end|><|start|>assistant<|channel|>final<|message|>Done.<|return|>",
        );

        assert!(parsed.has_completed_final());
        assert_eq!(
            parsed.reasoning_content(),
            Some("Need to answer carefully.".to_string())
        );
        assert_eq!(parsed.final_content(), Some("Done.".to_string()));
    }
}

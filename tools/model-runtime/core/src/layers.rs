...
impl Qwen3VLRotaryEmbedding {
    ...
    pub fn compute_cos_sin(&self, position_ids: &Tensor, dtype: DType) -> Result<(Tensor, Tensor)> {
        let rotary_device = self.inv_freq.device();
        let position_ids = if position_ids.device().same_device(rotary_device) {
            position_ids.clone()
        } else {
            position_ids.to_device(rotary_device)?
        };
...
impl RotaryEmbedding {
    ...
    pub fn forward(
        ...
        if cfg!(feature = "cuda") && qh == kh {
            ...
            engine_quant::rotary::apply_rotary_inplace(
                &q_embed,
                &k_embed,
                &cos,
                &sin,
...
impl GptOssRotaryEmbedding {
    ...
    pub fn forward(
        ...
        if q.device().is_cuda() && qh == k.dim(1)? {
            ...
            engine_quant::rotary::apply_rotary_inplace(&q_embed, &k_embed, &cos, &sin, true)?;

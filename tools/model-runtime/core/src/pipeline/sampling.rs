...
use crate::{
    harmony::{parse_harmony_response_text, HarmonyToolCall},
...
fn maybe_log_harmony_unparsed_delta(
    delta: &str,
    state: Option<String>,
    ...
    let _ = writeln!(
        f,
        "Harmony raw delta without parsed output: finish={:?} delta={:?} state={}",
        is_done,
        delta,
        state.unwrap_or_else(|| "unavailable".to_string())
    );
}

fn harmony_tool_calls_to_responses(tool_calls: Vec<HarmonyToolCall>) -> Vec<ToolCallResponse> {
    tool_calls
        .into_iter()
        .enumerate()
...
pub(crate) async fn finish_or_add_toks_to_seq(
    ...
    let include_special = seq.tools.is_some()
        || seq.is_harmony_mode()
        || seq.is_think_tag_mode()
        || seq.is_channel_tag_mode();
    ...
    let harmony_text_parse = if seq.is_harmony_mode() {
        let raw = String::from_utf8_lossy(seq.completion_bytes());
        Some(parse_harmony_response_text(raw.as_ref()))
    } else {
        None
    };
    ...
    if seq.is_harmony_mode() && seq.has_completed_harmony_tool_calls() {
        is_done = Some(StopReason::ToolCalls);
    }

    if seq.is_harmony_mode() && !seq.has_completed_harmony_tool_calls() {
        if let Some(parsed) = harmony_text_parse.as_ref() {
            if parsed.has_completed_tool_call() {
                is_done = Some(StopReason::ToolCalls);
            } else if parsed.has_completed_final() {
                is_done = Some(StopReason::Eos);
            }
        }
    }
                ...
                if let Some(delta) = crate::handle_seq_error_ok!(delta_result, seq.responder()) {
                    if seq.get_mut_group().is_chat {
                        ...
                        let (content_delta, reasoning_delta) = if seq.is_harmony_mode() {
                            ...
                            if final_delta.is_some() || reasoning.is_some() {
                                (final_delta, reasoning)
                            } else if is_done.is_some() {
                                (
                                    harmony_text_parse.as_ref().and_then(|p| p.final_content()),
                                    harmony_text_parse
                                        .as_ref()
                                        .and_then(|p| p.reasoning_content()),
                                )
                            } else {
                                (None, None)
                            }
                        } else if seq.is_channel_tag_mode() {
                            let content = seq.get_channel_tag_content_delta();
                            let reasoning = seq.get_channel_tag_reasoning_delta();
                            (content, reasoning)
                        ...
                        let tool_calls = if seq.is_harmony_mode() {
                            if is_done.is_some() {
                                let mut tool_calls =
                                    harmony_tool_calls_to_responses(seq.get_harmony_tool_calls());
                                if tool_calls.is_empty() {
                                    tool_calls = harmony_text_parse
                                        .as_ref()
                                        .map(|p| {
                                            harmony_tool_calls_to_responses(p.tool_calls.clone())
                                        })
                                        .unwrap_or_default();
                                }
                                if !tool_calls.is_empty() {
                        ...
                        let has_stream_delta = content_delta.is_some()
                        ...
                        } else if seq.is_harmony_mode() && !delta.is_empty() {
                            maybe_log_harmony_unparsed_delta(
                                &delta,
                                seq.get_harmony_debug_state(),
                                &is_done,
                            );
                        }
                    } else {
        ...
        */
        {
            ...
            // Finalize Gemma 4 channel parser if in channel-tag mode
            seq.channel_tag_finalize();
            ...
            if seq.get_mut_group().is_chat {
                ...
                let (text_new, tool_calls, reasoning_content) = if seq.is_harmony_mode() {
                    let fallback = parse_harmony_response_text(&text);
                    let final_content = seq
                        .get_harmony_final_content()
                        .or_else(|| fallback.final_content());
                    let reasoning = seq
                        .get_harmony_reasoning_content()
                        .or_else(|| fallback.reasoning_content());

                    // Get Harmony tool calls
                    let mut tool_calls =
                        harmony_tool_calls_to_responses(seq.get_harmony_tool_calls());
                    if tool_calls.is_empty() {
                        tool_calls = harmony_tool_calls_to_responses(fallback.tool_calls);
                    }

                    (final_content, tool_calls, reasoning)
                } else if seq.is_channel_tag_mode() {
                    seq.channel_tag_finalize();
                    let final_content = seq.get_channel_tag_content();
                    let reasoning = seq.get_channel_tag_reasoning_content();

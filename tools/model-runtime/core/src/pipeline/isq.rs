...
// Keep shards small enough that large-model CUDA quantization can flush
// incrementally instead of buffering nearly the whole artifact into one file.
#[cfg(target_pointer_width = "64")]
const MAX_UQFF_SIZE_BYTES: usize = 1024 * 1024 * 1024;
    ...
    fn quantize(
    ...
    ) -> candle_core::Result<()> {
        {
            ...
            if apply_quantization {
                ...
                if let Some(explicit_thread_limit) = env::var("ENGINE_ISQ_CPU_THREADS")
                    .ok()
                    .and_then(|value| value.parse::<usize>().ok())
                    .filter(|value| *value > 0)
                {
                    minimum_max_threads = explicit_thread_limit;
                } else if env::var("ENGINE_ISQ_SINGLETHREAD").is_ok() {
                ...
                let mut sync_devices = Vec::new();
                for (device, _) in &devices_and_dtypes {
                    if !sync_devices
                        .iter()
                        .any(|seen: &Device| seen.same_device(device))
                    {
                        sync_devices.push(device.clone());
                    }
                }
            ...
            if let Some(serialized) = write_artifacts {
                ...
                // On CUDA/Metal, we must serialize on the current thread; flush incrementally so
                // large models do not need one giant in-memory `Vec<(String, Vec<u8>)>` before the
                // first shard ever hits disk.
                ...
                for (i, (layer, _)) in tensors.iter().enumerate() {
                    ...
                    current_chunk.push((i.to_string(), tensor));
                    if current_bytes >= MAX_UQFF_SIZE_BYTES {
                        flush_uqff_shard(parent, &file_stem, shard_index, &mut current_chunk, false)?;
                        shard_index += 1;
                        current_bytes = 0;
                    }
                }
                ...
                flush_uqff_shard(parent, &file_stem, shard_index, &mut current_chunk, true)?;

...
use engine_quant::{ColumnParallelLayer, QuantMethod, RowParallelLayer, ShardedVarBuilder};
...
impl VisionAttention {
    fn new(dim: usize, num_heads: usize, vb: ShardedVarBuilder) -> Result<Self> {
        Ok(Self {
            qkv: engine_quant::linear(dim, dim * 3, &None, vb.pp("qkv"))?,
            proj: engine_quant::linear(dim, dim, &None, vb.pp("proj"))?,
            num_heads,

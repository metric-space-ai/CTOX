...
fn sinks_attn_regular(
    ...
    #[cfg(feature = "cuda")]
    if q.device().is_cuda() {
        return engine_paged_attn::flash_attn_sinks(
            q,
            k,
            v,
            Some(sinks),
    ...
    if q.device().is_metal() {
        return engine_quant::flash_attn_sinks_metal(
            q,
            k,
            v,
            Some(sinks),
...
fn sinks_attn_varlen(
    ...
    #[cfg(feature = "cuda")]
    if device.is_cuda() {
        return engine_paged_attn::flash_attn_sinks_varlen(
            q,
            &k_packed,
            &v_packed,
            Some(sinks),
    ...
    #[cfg(feature = "metal")]
    if device.is_metal() {
        return engine_quant::flash_attn_sinks_varlen_metal(
            q,
            &k_packed,
            &v_packed,
            Some(sinks),
...
fn sinks_attn_cpu(
    ...
    let att = engine_quant::softmax_with_sinks(&att, sinks, mask)?;
    MatMul.matmul(&att, &v)
}

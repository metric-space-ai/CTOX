...
fn parse_nvidia_smi_memory_to_bytes(raw: &str) -> Option<u64> {
    let mib: u64 = raw.trim().parse().ok()?;
    Some(mib.saturating_mul(1024 * 1024))
}

fn parse_nvidia_smi_device_query(raw: &str) -> Vec<DeviceInfo> {
    raw.lines()
        .filter_map(|line| {
            ...
            let ordinal: usize = columns[0].parse().ok()?;
            let compute_capability = parse_compute_capability(columns[4]);
            let flash_attn_compatible = compute_capability.map(|(major, _minor)| major >= 8);
            let flash_attn_v3_compatible =
                compute_capability.map(|(major, minor)| major == 9 && minor == 0);
            Some(DeviceInfo {
                kind: "cuda".to_string(),
                ordinal: Some(ordinal),
                name: Some(columns[1].to_string()),
                total_memory_bytes: parse_nvidia_smi_memory_to_bytes(columns[2]),
                available_memory_bytes: parse_nvidia_smi_memory_to_bytes(columns[3]),
                compute_capability,
                flash_attn_compatible,
                flash_attn_v3_compatible,
...
fn collect_nvidia_smi_devices() -> Vec<DeviceInfo> {
    ...
    parse_nvidia_smi_device_query(&stdout)
}
...
fn collect_devices(sys: &System) -> Vec<DeviceInfo> {
    ...
    if !devices.iter().any(|device| device.kind == "cuda") {
        devices.extend(collect_nvidia_smi_devices());
    }
...
#[cfg(test)]
mod tests {
    use super::parse_nvidia_smi_device_query;

    #[test]
    fn parses_nvidia_smi_device_rows() {
        let devices = parse_nvidia_smi_device_query(
            "0, NVIDIA RTX 4500, 24576, 22000, 8.9\n1, NVIDIA RTX 4500, 24576, 21000, 8.9\n",
        );

        assert_eq!(devices.len(), 2);
        assert_eq!(devices[0].ordinal, Some(0));
        assert_eq!(devices[0].name.as_deref(), Some("NVIDIA RTX 4500"));
        assert_eq!(devices[0].compute_capability, Some((8, 9)));
        assert_eq!(devices[0].flash_attn_compatible, Some(true));
        assert_eq!(devices[0].flash_attn_v3_compatible, Some(false));
        assert_eq!(devices[0].total_memory_bytes, Some(24_576 * 1024 * 1024));
        assert_eq!(devices[1].ordinal, Some(1));
    }

    #[test]
    fn ignores_invalid_nvidia_smi_rows() {
        let devices = parse_nvidia_smi_device_query(
            "not,a,valid,row\n0, NVIDIA RTX 4500, 24576, 22000, invalid\n",
        );

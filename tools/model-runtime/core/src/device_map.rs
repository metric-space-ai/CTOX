...
impl DeviceMapSetting {
    ...
    pub fn into_mapper(
        ...
        let resolved_nm_device = resolve_nm_device_for_visible_devices(device, all_devices);
        match self {
...
pub(crate) fn resolve_nm_device_for_visible_devices(
    default_device: &Device,
    all_devices: &[Device],
) -> Device {
    let ordinal = std::env::var("ENGINE_NM_DEVICE_ORDINAL")
        .ok()
    ...
    match ordinal {
        ...
        Some(ord) => {
            info!(
                "Ignoring invalid ENGINE_NM_DEVICE_ORDINAL={} for {} visible devices.",
                ord,
                all_devices.len()
            );
...
pub trait DeviceMapper: Debug {
    ...
    fn get_comm_for(&self, layer_idx: usize) -> Result<Arc<engine_quant::Comm>>;
...
impl DeviceMapper for LayerDeviceMapper {
    ...
    fn get_comm_for(&self, layer_idx: usize) -> Result<Arc<engine_quant::Comm>> {
        let id = engine_quant::Id::new();
        Ok(Arc::new(engine_quant::Comm::from_device(
            id,
...
pub struct NcclDeviceMapper {
    nm_device: Device,
    model_layers: usize,
    comm: Option<Arc<engine_quant::Comm>>,
}
...
pub struct NcclPipelineParallelMapper {
    mappings: Vec<(Arc<engine_quant::Comm>, Device)>,
    nm_device: Device,
}

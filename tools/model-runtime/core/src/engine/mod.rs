...
impl Engine {
    ...
    fn replicate_request_to_daemons(&self, request: &Request) {
        if !distributed::is_daemon() && engine_quant::distributed::use_nccl() {
            let name = distributed::ipc_name().unwrap();
            let num_workers =
                engine_quant::distributed::get_global_tp_size_from_devices().unwrap() - 1;

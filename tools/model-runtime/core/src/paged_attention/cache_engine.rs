...
impl CacheEngine {
    ...
    fn calculate_key_block_shape(
        ...
        let (head_bytes, x) = if let Some(bits) = cache_type.turboquant_bits() {
            (bits.packed_key_bytes_for_dim(k_head_dim), 1usize)
        } else {
            let element_size = dtype.size_in_bytes();
            let x = 16 / element_size;
            (k_head_dim, x)
        };
        (num_kv_heads, head_bytes.div_ceil(x), block_size, x)
    }
    ...
    fn calculate_value_block_shape(
        ...
        let value_bytes = if let Some(bits) = cache_type.turboquant_bits() {
            bits.packed_value_bytes_for_dim(v_head_dim)
        } else {
            v_head_dim
        };
        (num_kv_heads, value_bytes, block_size)
    }
}
...
mod tests {
    ...
    struct MixedHeadDimConfig;

    impl ModelConfigLike for MixedHeadDimConfig {
        fn max_seq_len(&self) -> usize {
            4096
        }
    ...
    fn turboquant_block_shapes_include_packed_overhead() {
        ...
        let value_shape = CacheEngine::calculate_value_block_shape(
            &DummyConfig,
            0,
            PagedCacheType::TurboQuant3,
            32,
    ...
    fn dense_block_shapes_stay_unchanged() {
        ...
        let value_shape =
            CacheEngine::calculate_value_block_shape(&DummyConfig, 0, PagedCacheType::Auto, 32);
    ...
    #[test]
    fn per_layer_turboquant_shapes_follow_layer_geometry() {
        let key_shape = CacheEngine::calculate_key_block_shape(
            &MixedHeadDimConfig,
            1,
            PagedCacheType::TurboQuant3,
        ...
        assert_eq!(key_shape, (2, 224, 32, 1));
        assert_eq!(value_shape, (2, 224, 32));
    }
}

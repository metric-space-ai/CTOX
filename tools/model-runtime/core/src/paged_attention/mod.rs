...
macro_rules! mb_to_blocks {
    ($mb_size:expr, $block_bytes:expr, $block_size:expr, $config:expr, $cache_type:expr) => {
        $mb_size
            / $block_bytes
            / $block_size
            / $config.num_layers()
            / kv_cache_storage_units_per_token($config, $cache_type)
    };
}

macro_rules! ctxt_to_blocks {
    ($context_len:expr, $block_bytes:expr, $block_size:expr, $config:expr, $cache_type:expr) => {
        $context_len
            * $block_bytes
            * $config.num_layers()
            * kv_cache_storage_units_per_token($config, $cache_type)
...
fn kv_cache_storage_units_per_token(
    config: &dyn ModelConfigLike,
    cache_type: PagedCacheType,
) -> usize {
    if let Some(bits) = cache_type.turboquant_bits() {
        let num_layers = config.num_layers().max(1);
        let total: usize = (0..num_layers)
            .map(|layer_idx| {
                config.num_kv_heads_for_layer(layer_idx)
                    * (bits.packed_key_bytes_for_dim(config.k_head_dim_for_layer(layer_idx))
                        + bits.packed_value_bytes_for_dim(config.v_head_dim_for_layer(layer_idx)))
            })
            .sum();
        total / num_layers
    } else {
...
pub fn calculate_cache_config(
    ...
    let storage_unit_size = dtype.size_in_bytes();
        ...
        let mem_gpu = match mem_gpu {
            ...
            MemoryGpuConfig::Utilization(f) => {
                if model_weight_size_in_bytes.is_some() {
                    ...
                    // Post-load, utilization should scale the actually free memory budget.
                    // This avoids collapsing to zero KV cache on hosts where the loaded
                    // model legitimately occupies more than `f * total_memory`.
                    (free * f).max(0.0) as usize
                }
            }
            MemoryGpuConfig::ContextSize(toks) => {
                ...
                ctxt_to_blocks!(toks, storage_unit_size, block_size, config, cache_type)
                    / SIZE_IN_MB
            }
        };
    ...
    if device.is_metal() {
        ...
        let mem_for_tokens = ctxt_to_blocks!(
            max_tokens,
            storage_unit_size,
            block_size,
            config,
    ...
    let num_gpu_blocks = mb_to_blocks!(
        mem_gpu * SIZE_IN_MB,
        storage_unit_size,
        block_size,
        config,
...
pub fn explicit_paged_cache_type(config: Option<PagedAttentionConfig>) -> Option<PagedCacheType> {
    config.map(|config| config.cache_type)
}

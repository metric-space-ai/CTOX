...
use crate::{
    ...
    paged_attention::{
        AttentionImplementation, ModelConfigMetadata, PagedAttention, PagedCacheType,
    },
    pipeline::{
        extract_logits,
...
fn glm_debug_enabled() -> bool {
    std::env::var_os("ENGINE_GLM_DEBUG").is_some()
}

fn glm_debug_log(message: impl AsRef<str>) {
    if glm_debug_enabled() {
        tracing::info!(target: "engine_core::models::glm4_moe_lite_debug", "{}", message.as_ref());
    }
}

fn glm_uses_quantized_paged_cache(cache_type: Option<PagedCacheType>) -> bool {
    matches!(
        cache_type,
        Some(
...
fn glm_can_use_mla_paged_kv(
    attention_mechanism: AttentionImplementation,
    device: &Device,
    paged_attn_cache_type: Option<PagedCacheType>,
) -> bool {
    matches!(attention_mechanism, AttentionImplementation::PagedAttention)
        ...
        && !glm_uses_quantized_paged_cache(paged_attn_cache_type)
}
...
fn glm_non_repeating_devices(
    ...
    for layer_idx in 0..num_hidden_layers {
        ...
        if let Some((_, count)) = counts
            .iter_mut()
            .find(|(seen, _)| seen.same_device(&device))
        {
            *count += 1;
        } else {
...
impl Attention {
    ...
    fn new(
        ...
        let use_mla_paged_kv = paged_attn
            .as_ref()
            .is_some_and(|paged_attn| !glm_uses_quantized_paged_cache(paged_attn.cache_type()));
        let mla_weights = MlaWeights::new(use_mla_paged_kv, Some(&layer_device));
    ...
    fn forward(
        ...
        let mut attn_out = if use_mla_decode {
            ...
            let use_mla_cache = should_use_mla_cache(self.use_mla_paged_kv, q.device());
            if glm_debug_enabled() {
                glm_debug_log(format!(
                    "attention.non_mla.cache seq_len={seq_len} decode={is_decode} use_mla_cache={use_mla_cache} paged_attn={} mla_paged_kv={}",
                    self.paged_attn.is_some(),
                    self.use_mla_paged_kv
                ));
            }
        ...
        if glm_debug_enabled() {
            glm_debug_log(format!(
                "attention.exit bs={bs} seq_len={seq_len} decode={is_decode}"
            ));
        }
...
impl Moe {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if glm_debug_enabled() {
            glm_debug_log(format!(
                "moe.enter bs={b_size} seq_len={seq_len} hidden={hidden_dim}"
            ));
        }
...
impl Glm4MoeLite {
    pub fn new(
        ...
        tracing::info!(
            target: "engine_core::models::glm4_moe_lite",
            "GLM4-MoE-Lite constructor start: loading_isq={} embed_device={:?} head_device={:?}",
            normal_loading_metadata.loading_isq,
            embed_device.location(),
            head_device.location()
        );
        ...
        tracing::info!(
            target: "engine_core::models::glm4_moe_lite",
            "GLM4-MoE-Lite shared components initialized; building {} decoder layers.",
            cfg.num_hidden_layers
        );
        ...
        .par_iter_if_isq(|layer_idx| {
            ...
            .inspect(|_| {
                ...
                tracing::info!(
                    target: "engine_core::models::glm4_moe_lite",
                    "GLM4-MoE-Lite decoder layer {layer_idx} constructed."
                );
            })
        })?;
        ...
        tracing::info!(
            target: "engine_core::models::glm4_moe_lite",
            "GLM4-MoE-Lite decoder layers constructed."
        );
        let use_mla_paged_kv = glm_can_use_mla_paged_kv(
            attention_mechanism,
            &normal_loading_metadata.real_device,
            normal_loading_metadata.paged_attn_cache_type,
        );
        ...
        Ok(Self {
            ...
            cfg: ModelConfigMetadata {
                ...
                kv_cache_layout: if use_mla_paged_kv {
        ...
        .inspect(|_| {
            tracing::info!(
                target: "engine_core::models::glm4_moe_lite",
                "GLM4-MoE-Lite model construction complete."
            );
        })
    }
    ...
    pub fn forward(
        ...
        if glm_debug_enabled() {
            let is_decode =
                metadata.is_some() && input_ids.dims2().map(|(_, s)| s == 1).unwrap_or(false);
            glm_debug_log(format!(
...
#[cfg(test)]
mod tests {
    use super::glm_uses_quantized_paged_cache;
    use crate::paged_attention::PagedCacheType;

    #[test]
    fn glm_treats_fp8_cache_as_quantized() {
        assert!(glm_uses_quantized_paged_cache(Some(PagedCacheType::F8E4M3)));
    }

    #[test]
    fn glm_leaves_auto_cache_unquantized() {
        assert!(!glm_uses_quantized_paged_cache(None));
    }
}

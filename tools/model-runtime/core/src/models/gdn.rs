...
/// Chunked gated delta rule — candle-ops port of llama.cpp's
/// `llm_build_delta_net_base::build_delta_net_chunking` (via the dflash
/// reference's `dflash/src/delta_net_chunked.cpp`).
///
/// Why: at `n_tokens > 1` (DFlash chain verify feed = B+1 = 17 tokens,
/// or any prefill chunk) the sequential per-token recurrence above
/// serialises the only dimension where we have parallelism to exploit.
/// Reformulating the recurrence as a sequence of matmul / cumsum / tril
/// ops makes ~90% of the compute fall into cuBLAS batched GEMM, which
/// the A6000 runs at ~99% of theoretical peak. On A6000 the reference
/// reports `verify_compute ≈ 48 ms` for 48 GDN layers × 23-token verify;
/// our sequential-recurrence path spends several hundred ms on the same
/// shape because every per-token iteration is its own set of tiny
/// kernel launches.
///
/// Scope: chain mode (no DDTree tree with parent_ids), no per-token
/// intermediate-state capture. The caller stays on
/// `gated_delta_rule_recurrence` for seq_len==1 pure decode (there's
/// nothing to parallelise) and for any path that needs per-token SSM
/// snapshots for speculative rollback.
///
/// Inputs (matching `gated_delta_rule_recurrence`):
///   q, k: `(batch, seq, num_v_heads, head_k_dim)`
///   v:    `(batch, seq, num_v_heads, head_v_dim)`
///   g:    `(batch, seq, num_v_heads)` — scalar gate per head (GDA form)
///   beta: `(batch, seq, num_v_heads)`
///   state: `(batch, num_v_heads, head_k_dim, head_v_dim)`, mutated in
///          place with the post-chunk state.
///
/// Returns: `(batch, seq, num_v_heads, head_v_dim)`.
...
pub fn gated_delta_rule_chunked(
    ...
    // Shape probe.
    let (b, seq, n_heads, s_k) = q.dims4()?;
    let s_v = v.dim(3)?;
    if seq == 0 {
        // Degenerate — no work, return a correctly-shaped zero tensor.
        return Tensor::zeros((b, seq, n_heads, s_v), out_dtype, &device);
    }
    if seq == 1 {
        // Single-token forward has no intra-chunk parallelism to exploit
        // and all ops below would reduce to scalar per-head matmuls; the
        // tiled sequential recurrence wins here. Dispatch back.
        return gated_delta_rule_recurrence(q, k, v, g, beta, state);
    }

    // Chunk size. The reference uses CS=64 for GDA (scalar gate) which
    // is the Qwen3.5 config we target. For seq < CS we degenerate to a
    // single chunk; the chunked algorithm still does strictly less work
    // than `seq` per-token iterations because the inner chunk is
    // triangular-matrix-dominant.
    const CS: usize = 64;
    let pad = (CS - seq % CS) % CS;
    let seq_padded = seq + pad;
    let n_chunks = seq_padded / CS;

    // Work dtype: all intermediates run in f32 (reference uses f32 end
    // to end for the chunked path; the outputs widen/narrow at the
    // function boundary).
    ...
    // Scale q, then permute everything into head-major layout so the
    // subsequent chunk-reshape treats (batch, head) as batch and
    // (n_chunks, CS, feat) as the inner work.
    //   (b, seq, n_heads, s_k) -> (b, n_heads, seq, s_k)
    let q = (q.transpose(1, 2)?.contiguous()? * scale as f64)?;
    let k = k.transpose(1, 2)?.contiguous()?;
    let v = v.transpose(1, 2)?.contiguous()?;
    // g, beta: (b, seq, h) -> (b, h, seq)
    let g_bhs = g.transpose(1, 2)?.contiguous()?;
    let beta_bhs = beta.transpose(1, 2)?.contiguous()?;

    // Pad along seq dim with zeros (q, k, v) / zeros (g, beta). Zero
    // pad is safe: zero q/k contributes 0 to kb/kq, zero beta zeroes
    // the delta, zero g decays by 1 — padded rows don't influence
    // real rows because the triangular mask blocks them.
    let pad_q = pad_seq_dim(&q, pad, 2)?;
    let pad_k = pad_seq_dim(&k, pad, 2)?;
    let pad_v = pad_seq_dim(&v, pad, 2)?;
    let pad_g = pad_seq_dim(&g_bhs, pad, 2)?;
    let pad_beta = pad_seq_dim(&beta_bhs, pad, 2)?;

    let (q, k, v) = (pad_q, pad_k, pad_v);
    let g_bhs = pad_g;
    let beta_bhs = pad_beta;

    // v_b = v * beta;  k_b = k * beta  (broadcast beta over feat dim).
    let beta_bhs_u = beta_bhs.unsqueeze(D::Minus1)?; // (b, h, seq, 1)
    let v_b = v.broadcast_mul(&beta_bhs_u)?;
    let k_b = k.broadcast_mul(&beta_bhs_u)?;

    // Reshape into chunks. Flatten (b, h) into a single "batch" dim and
    // split seq into (n_chunks, CS). Final shape for all five tensors:
    //   q, k, k_b:   (b*h, n_chunks, CS, s_k)
    //   v, v_b:      (b*h, n_chunks, CS, s_v)
    //   g, beta:     (b*h, n_chunks, CS)
    let bh = b * n_heads;
    let q = q.reshape((bh, n_chunks, CS, s_k))?;
    let k = k.reshape((bh, n_chunks, CS, s_k))?;
    let k_b = k_b.reshape((bh, n_chunks, CS, s_k))?;
    let v = v.reshape((bh, n_chunks, CS, s_v))?;
    let v_b = v_b.reshape((bh, n_chunks, CS, s_v))?;
    let g_bhs = g_bhs.reshape((bh, n_chunks, CS))?;

    // Cumulative sum of gates within each chunk: g_cs[..., t] = sum_{t' <= t} g[..., t']
    // Shape (bh, n_chunks, CS).
    let g_cs = g_bhs.cumsum(2)?;

    // Lower-triangular decay mask:  decay_ij = exp(g_cs_j - g_cs_i)
    // for j <= i, zero above diagonal. Shape (bh, n_chunks, CS, CS).
    let tril = Tensor::tril2(CS, DType::F32, &device)?
        .reshape((1, 1, CS, CS))?; // (1, 1, CS, CS) broadcastable
    let g_cs_i = g_cs.unsqueeze(3)?; // (bh, n_chunks, CS, 1)
    let g_cs_j = g_cs.unsqueeze(2)?; // (bh, n_chunks, 1, CS)
    let decay_mask = g_cs_j.broadcast_sub(&g_cs_i)?;
    // Zero out above-diagonal BEFORE exp (exp(0) = 1 on the upper tri
    // rows is harmless because they get masked again below via tril).
    let decay_mask = decay_mask.broadcast_mul(&tril)?.exp()?;
    let decay_mask = decay_mask.broadcast_mul(&tril)?; // enforce lower-tri

    // kb = (k @ k_b^T) ⊙ decay_mask  — shape (bh, n_chunks, CS, CS).
    // Using k.matmul(k_b.T) with k: (..., CS, s_k) and k_b.T: (..., s_k, CS).
    let kb_pre = k.matmul(&k_b.transpose(2, 3)?.contiguous()?)?;
    let kb = kb_pre.broadcast_mul(&decay_mask)?;

    // kq = (k @ q^T) ⊙ decay_mask, then lower-triangular masked.
    // Same layout as kb.
    let kq_pre = k.matmul(&q.transpose(2, 3)?.contiguous()?)?;
    let kq = kq_pre
        .broadcast_mul(&decay_mask)?
        .broadcast_mul(&tril)?;

    // attn = lower_tri(kb) (strictly lower; diagonal zeroed).
    //   lhs = I + attn      (lower-tri with unit diag)
    //   lin_solve = (I + attn)^{-1} · (-attn)
    //   final_attn = lin_solve + I
    // Since `attn` is nilpotent of index ≤ CS (strict lower tri CS×CS),
    // the Neumann series (I + attn)^{-1} = I − attn + attn² − … + (−attn)^{CS-1}
    // terminates cleanly. We compute `final_attn = I + lin_solve` by
    // accumulating `X ← I; P ← I; for k in 1..CS: P = (-attn) · P; X = X + P`
    // so X = I − attn + attn² − attn³ + … which equals (I + attn)^{-1}.
    //
    // That gives us `(I + attn)^{-1}` explicitly; the reference then
    // multiplies it by `-attn` and adds I — but the resulting identity
    // `(I + attn)^{-1}·(-attn) + I = (I + attn)^{-1}` simplifies when
    // you multiply out: `final_attn = (I+attn)^{-1}`. So we're done
    // after the Neumann loop.
    let strict_lower =
        Tensor::tril2(CS, DType::F32, &device)?
            .sub(&Tensor::eye(CS, DType::F32, &device)?)?
            .reshape((1, 1, CS, CS))?; // (1,1,CS,CS), 1s strictly below diagonal
    let attn_strict = kb.broadcast_mul(&strict_lower)?;
    let neg_attn = attn_strict.neg()?;
    let identity = Tensor::eye(CS, DType::F32, &device)?
        .reshape((1, 1, CS, CS))?;

    // Truncate the Neumann series once |attn|^k is numerically zero.
    // For strict-lower attn, attn^CS = 0 exactly (nilpotent), so CS-1
    // iterations suffice. We keep that upper bound.
    let mut final_attn = identity.broadcast_as(kb.shape())?.contiguous()?;
    let mut power = identity.broadcast_as(kb.shape())?.contiguous()?;
    for _ in 1..CS {
        power = power.matmul(&neg_attn)?; // P ← P · (-attn)  (order: monomial on the right)
        final_attn = final_attn.add(&power)?;
    }

    // v_new  = (v_b^T @ final_attn^T)^T, written simply as
    // v_new = final_attn · v_b            (shape (bh, n_chunks, CS, s_v))
    // (attn is the chunk-local "attention-to-past-in-chunk" matrix).
    let v_new = final_attn.matmul(&v_b)?;

    // g_exp   = exp(g_cs)                 shape (bh, n_chunks, CS)
    // kbg     = k_b * g_exp (broadcast over s_k)
    // k_cd    = kbg^T @ final_attn        (bh, n_chunks, s_k, CS) — per-chunk
    //           state update contributions before applying to the running s.
    let g_exp = g_cs.exp()?; // (bh, n_chunks, CS)
    let kbg = k_b.broadcast_mul(&g_exp.unsqueeze(D::Minus1)?)?;
    let k_cd = kbg.transpose(2, 3)?.contiguous()?.matmul(&final_attn)?;

    // q_g_exp = q ⊙ g_exp[..., :, None]   (bh, n_chunks, CS, s_k)
    let q_g_exp = q.broadcast_mul(&g_exp.unsqueeze(D::Minus1)?)?;

    // g_last is the last-position cumulative gate of each chunk,
    // g_diff = -(g_cs - g_last), kg = k ⊙ exp(g_diff)          (bh, n_chunks, CS, s_k)
    let g_last = g_cs.narrow(2, CS - 1, 1)?; // (bh, n_chunks, 1)
    let g_diff = g_last
        .broadcast_sub(&g_cs)?; // (bh, n_chunks, CS) — positive decay toward the chunk end
    let g_diff_exp = g_diff.exp()?;
    let kg = k.broadcast_mul(&g_diff_exp.unsqueeze(D::Minus1)?)?;

    // g_last_exp: (bh, n_chunks, 1) — multiplicative state decay across a chunk.
    let g_last_exp = g_last.exp()?;

    // Running state s, reshaped to (bh, s_k, s_v) — one matrix per
    // head per batch element.
    let mut s = state
        .to_dtype(DType::F32)?
        .reshape((bh, s_k, s_v))?;

    // Allocate the output in the same flat layout.
    let mut chunk_outs: Vec<Tensor> = Vec::with_capacity(n_chunks);

    for c in 0..n_chunks {
        // k_cd is (bh, n_chunks, s_k, CS); narrow on the chunk dim.
        let ch_k_cd = k_cd.narrow(1, c, 1)?.squeeze(1)?.contiguous()?; // (bh, s_k, CS)
        let ch_v_new = v_new.narrow(1, c, 1)?.squeeze(1)?.contiguous()?; // (bh, CS, s_v)
        let ch_kq = kq.narrow(1, c, 1)?.squeeze(1)?.contiguous()?; // (bh, CS, CS)
        let ch_q_g_exp = q_g_exp.narrow(1, c, 1)?.squeeze(1)?.contiguous()?; // (bh, CS, s_k)
        let ch_kg = kg.narrow(1, c, 1)?.squeeze(1)?.contiguous()?; // (bh, CS, s_k)
        let ch_g_last_exp = g_last_exp
            .narrow(1, c, 1)?
            .squeeze(1)?
            .contiguous()?; // (bh, 1)

        // v_p = k_cd^T @ s          — (bh, CS, s_k) · (bh, s_k, s_v) = (bh, CS, s_v)
        let v_p = ch_k_cd.transpose(1, 2)?.contiguous()?.matmul(&s)?;
        // v_t_new = v_new - v_p    — (bh, CS, s_v)
        let v_t_new = ch_v_new.sub(&v_p)?;
        // v_attn = kq @ v_t_new   — (bh, CS, CS) · (bh, CS, s_v) = (bh, CS, s_v)
        let v_attn = ch_kq.matmul(&v_t_new)?;
        // attn_inter = q_g_exp @ s   — (bh, CS, s_k) · (bh, s_k, s_v) = (bh, CS, s_v)
        let attn_inter = ch_q_g_exp.matmul(&s)?;
        // output for this chunk: attn_inter + v_attn.
        let o_ch = attn_inter.add(&v_attn)?; // (bh, CS, s_v)
        chunk_outs.push(o_ch);

        // State update:
        //   kgv = kg^T @ v_t_new                    (bh, s_k, s_v)
        //   s   = s * g_last_exp.unsqueeze  + kgv    — multiplicative across-chunk decay
        let kgv = ch_kg.transpose(1, 2)?.contiguous()?.matmul(&v_t_new)?;
        // broadcast g_last_exp (bh, 1) → (bh, 1, 1) for matrix multiplication shape.
        let g_scale = ch_g_last_exp.reshape((bh, 1, 1))?;
        s = s.broadcast_mul(&g_scale)?.add(&kgv)?;
    }

    // Stack chunks back to (bh, n_chunks, CS, s_v) then reshape to
    // (bh, seq_padded, s_v) and truncate padding.
    let out_chunks = Tensor::stack(&chunk_outs, 1)?; // (bh, n_chunks, CS, s_v)
    let out_flat = out_chunks.reshape((bh, seq_padded, s_v))?;
    ...
    // Reshape (bh, seq, s_v) -> (b, h, seq, s_v) -> (b, seq, h, s_v) and
    // cast back to caller dtype.
    let out = out_unpadded
        .reshape((b, n_heads, seq, s_v))?
        .transpose(1, 2)?
        .contiguous()?
        .to_dtype(out_dtype)?;

    *state = s.reshape((b, n_heads, s_k, s_v))?.to_dtype(state.dtype())?;

    Ok(out)
}

/// Pad tensor `t` along dim `dim` by `pad` zeros at the end. Helper
/// for the chunked-GDN's pre-reshape step; candle has `Tensor::pad`
/// only on the last two dims, so we express it via `cat` with a
/// zeros-tensor shaped like the pad region. `pad == 0` short-circuits.
fn pad_seq_dim(t: &Tensor, pad: usize, dim: usize) -> Result<Tensor> {
    ...
    pad_shape[dim] = pad;
    let zeros = Tensor::zeros(pad_shape, t.dtype(), t.device())?;

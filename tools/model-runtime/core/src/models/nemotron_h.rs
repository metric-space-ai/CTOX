...
impl RoutedExperts {
    fn apply_routed_expert_isq(
        ...
        let Some(params) = engine_quant::get_immediate_isq() else {
            return Ok(layer);
        };
        let Some(ImmediateIsqMatch { ty, device }) = engine_quant::immediate_isq_match(&vb) else {
            return Ok(layer);
        };
    ...
    fn new(
        ...
        let load_experts_vb =
            if engine_quant::get_immediate_isq().is_some() && !experts_vb.device().is_cpu() {
...
impl MambaLayer {
    ...
    fn forward_prefill(
        ...
        if seq_len >= self.conv_kernel_size {
            cache.conv_state = hidden_states
                .narrow(
                    D::Minus1,
...
impl Attention {
    fn load(
        ...
        let kv_shard =
            engine_quant::compute_kv_shard(cfg.num_key_value_heads(), cfg.head_dim, comm);

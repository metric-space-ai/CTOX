...
fn gptoss_swiglu(gate: &Tensor, up: &Tensor, alpha: f32, limit: f32) -> Result<Tensor> {
    ...
    if gate.device().is_cuda() {
        return engine_quant::gptoss_swiglu_fused(gate, up, alpha, limit);
    }
...
impl Attention {
    ...
    fn new(
        ...
        let (local_sink_offset, local_num_heads) =
            engine_quant::shard_bounds(num_heads, comm.rank(), comm.world_size());
...
impl GptOssMoE {
    ...
    fn forward(&self, xs: &Tensor, _layer_idx: usize) -> Result<Tensor> {
        ...
        let activated = {
            ...
            let result = engine_quant::gptoss_swiglu_interleaved(
                &gate_up_for_kernel,

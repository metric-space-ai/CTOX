use crate::{
    channel_tags::ChannelTagContext,
...
pub struct Sequence {
    ...
    // Gemma 4 channel-tag parsing context
    channel_tag_context: Option<ChannelTagContext>,
}
...
impl Sequence {
    ...
    pub fn add_token(
        ...
        if let Some(ref mut channel_ctx) = self.channel_tag_context {
            if !stopped_by_token {
                channel_ctx.process_bytes(&completion_bytes);
            }
        }
    ...
    /// Check if Harmony mode has detected any fully completed tool calls.
    pub fn has_completed_harmony_tool_calls(&self) -> bool {
        self.harmony_context
            .as_ref()
            .is_some_and(|ctx| ctx.has_completed_tool_calls())
    }
    ...
    pub fn get_harmony_debug_state(&self) -> Option<String> {
        self.harmony_context.as_ref().map(|ctx| ctx.debug_state())
    }
    ...
    // === Gemma 4 Channel Tag Support ===

    pub fn enable_channel_tag_mode(&mut self, starts_with_implicit_thinking: bool) {
        if self.channel_tag_context.is_none() {
            self.channel_tag_context = Some(if starts_with_implicit_thinking {
                ChannelTagContext::new_with_implicit_thinking()
            } else {
                ChannelTagContext::new()
            });
        }
    }

    pub fn is_channel_tag_mode(&self) -> bool {
        self.channel_tag_context.is_some()
    }

    pub fn get_channel_tag_reasoning_delta(&mut self) -> Option<String> {
        self.channel_tag_context
            .as_mut()
            .and_then(|ctx| ctx.get_reasoning_delta())
    }

    pub fn get_channel_tag_content_delta(&mut self) -> Option<String> {
        self.channel_tag_context
            .as_mut()
            .and_then(|ctx| ctx.get_content_delta())
    }

    pub fn get_channel_tag_reasoning_content(&self) -> Option<String> {
        self.channel_tag_context
            .as_ref()
            .and_then(|ctx| ctx.reasoning_content())
    }

    pub fn get_channel_tag_content(&self) -> Option<String> {
        self.channel_tag_context
            .as_ref()
            .and_then(|ctx| ctx.content())
    }

    pub fn channel_tag_finalize(&mut self) {
        if let Some(ref mut ctx) = self.channel_tag_context {
            ctx.finalize();
        }
    }
}

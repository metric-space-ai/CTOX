...
use engine_quant::ShardedVarBuilder;
...
use crate::{
    ...
    paged_attention::{AttentionImplementation, PagedCacheType},
...
impl<R: std::io::Seek + std::io::Read> Config<ParamsGGUF<'_, R>, Adapter<'_>> {
    pub fn try_into_model<T: FromAdapterGGUF>(self) -> Result<T, candle_core::Error> {
        // Destructure props:
        let ParamsGGUF(ct, Device { device, mapper }, _attention_implementation, _, dtype) =
            self.quant;

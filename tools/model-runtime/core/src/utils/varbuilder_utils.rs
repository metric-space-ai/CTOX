...
use engine_quant::{safetensors::MmapedSafetensors, ShardedSafeTensors, ShardedVarBuilder};
use regex::Regex;
...
const ENGINE_NO_MMAP: &str = "ENGINE_NO_MMAP";
...
pub(crate) fn from_mmaped_safetensors(
    ...
    let use_no_mmap = std::env::var(ENGINE_NO_MMAP).is_ok_and(|x| x == "1");
        ...
        if !iter.is_empty() {
            for (load_name, key_name) in iter.into_iter().with_progress(is_silent) {
                if !make_dummy_predicate(&load_name) {
                    ...
                    // If making a dummy, don't add the tensor. `engine_quant` handles this!

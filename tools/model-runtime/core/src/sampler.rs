...
use engine_quant::{CumSumOp, SortOp};
...
/// use engine_core::CustomLogitsProcessor;
...
fn sanitize_logits_tensor(logits: Tensor) -> Result<Tensor> {
    let logits = logits.to_dtype(DType::F32)?;
    let finite_mask = logits.eq(&logits)?;
    let logits = finite_mask.where_cond(&logits, &Tensor::zeros_like(&logits)?)?;
    logits.clamp(-1e4, 1e4)
}

fn sanitize_probabilities_in_place(probs: &mut [f32]) -> bool {
    let mut sum = 0.0f64;
...
impl Sampler {
    ...
    fn sample_multinomial(
        ...
        if !sanitize_probabilities_in_place(&mut probs) {
            let next_token = argmax_f32(&probs);
            let top_logprobs = if return_logprobs {
                Some(self.get_top_logprobs(&probs)?)
            } else {
    ...
    fn sample_top_kp_min_p(
        ...
        sanitize_probabilities_in_place(probs);
    ...
    pub fn sample(
        ...
        logits = sanitize_logits_tensor(logits)?;
        let next_token = if sample_speculative {
            match self.temperature {
                ...
                Some(temperature) => {
                    ...
                    let probs = candle_nn::ops::softmax_last_dim(&sanitize_logits_tensor(logits)?)?;
...
mod tests {
    ...
    fn test_argmax_ignores_nan() {
        assert_eq!(super::argmax_f32(&[f32::NAN, 0.25, 0.5]), 2);
        assert_eq!(super::argmax_f32(&[f32::NAN, f32::NEG_INFINITY]), 0);
    }

    #[test]
    fn test_sanitize_probabilities_renormalizes() {
        let mut probs = [0.5, f32::NAN, -1.0, 0.5];
        assert!(super::sanitize_probabilities_in_place(&mut probs));

...
pub fn mul_and_act(a: &Tensor, b: &Tensor, act: Activation) -> Result<Tensor> {
    if !a.device().same_device(b.device()) {
        candle_core::bail!(
            "mul_and_act device mismatch: a={:?} b={:?}",
            a.device().location(),
            b.device().location()
        );
    }
    ...
    if matches!(a.dtype(), DType::F16 | DType::BF16 | DType::F32) && a.dtype() == b.dtype() {
        ...
        let glu_act = match act {
            Activation::Silu | Activation::Swish => Some(engine_quant::GluActivationType::Silu),
            Activation::NewGelu | Activation::GeluPytorchTanh => {
                Some(engine_quant::GluActivationType::Gelu)
            }
            Activation::Gelu => Some(engine_quant::GluActivationType::GeluErf),
            Activation::Relu => Some(engine_quant::GluActivationType::Relu),
            _ => None,
        };

        if let Some(activation_type) = glu_act {
            return engine_quant::fused_glu(a, b, activation_type);
        }
    }

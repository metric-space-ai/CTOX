...
use core::ffi::c_char;
pub use engine_quant::distributed::{use_nccl, use_ring};
use engine_quant::{RingConfig, ShardedVarBuilder};
...
pub(crate) const IS_DAEMON_FLAG: &str = "__ENGINE_DAEMON_INTERNAL";
...
pub(crate) fn ipc_name() -> anyhow::Result<Name<'static>> {
    let printname = "engine_daemon.sock";
...
pub(crate) fn prepare_distributed_mapper<T: DeviceMappedModelLoader + IsqModelLoader + ?Sized>(
    ...
    let global_world_size = if let Ok(x) = std::env::var("ENGINE_MN_GLOBAL_WORLD_SIZE") {
        usize::from_str(&x).context("ENGINE_MN_GLOBAL_WORLD_SIZE")?
    } else {
        // global world size is always >= local world size
        std::cmp::max(
            engine_quant::distributed::get_global_tp_size_from_devices()?,
            local_world_size,
        )
    };

    let use_multi_node = std::env::var("ENGINE_MN_GLOBAL_WORLD_SIZE").is_ok();
    if use_multi_node {
        info!("ENGINE_MN_GLOBAL_WORLD_SIZE is set, entering multi-node.");
    }
    ...
    let local_rank = if let Ok(payload) = env::var(IS_DAEMON_FLAG) {
        ...
        id = engine_quant::Id::new();
        let num_ranks = engine_quant::distributed::get_global_tp_size_from_devices()?;
        ...
        for worker_rank in 0..num_workers {
            ...
            let debug_daemon_io = env::var("ENGINE_DEBUG_DAEMON_IO")
                .ok()
                .is_some_and(|value| value == "1");
            if debug_daemon_io {
                tracing::info!("Keeping NCCL worker stdio attached for debug output.");
    ...
    if use_multi_node {
        if let Ok(n_nodes) = env::var("ENGINE_MN_HEAD_NUM_WORKERS") {
            let n_nodes = usize::from_str(&n_nodes).context("ENGINE_MN_HEAD_NUM_WORKERS")?;
            info!("Head node managing {n_nodes} workers.");
            let Ok(port) = env::var("ENGINE_MN_HEAD_PORT") else {
                anyhow::bail!("Got ENGINE_MN_HEAD_NUM_WORKERS, expected ENGINE_MN_HEAD_PORT");
            };
            info!("Head node initializing connection on {port}.");
            let server =
                engine_quant::Server::new(&format!("0.0.0.0:{port}"), n_nodes, local_world_size)?;

            server.broadcast_id(&id)?;
        } else if let Ok(addr) = env::var("ENGINE_MN_WORKER_SERVER_ADDR") {
            info!("Worker node connecting to {addr}.");
            let client = engine_quant::Client::new(addr.parse()?, local_world_size)?;

            id = client.receive_id()?;
        }
    }

    let rank_offset = if env::var("ENGINE_MN_WORKER_SERVER_ADDR").is_ok() {
        let Ok(node_id) = env::var("ENGINE_MN_WORKER_ID") else {
            anyhow::bail!("Got ENGINE_MN_WORKER_SERVER_ADDR, expected ENGINE_MN_WORKER_ID");
        };
        let node_id = usize::from_str(&node_id).context("ENGINE_MN_WORKER_ID")?;
    ...
    let comm =
        engine_quant::Comm::from_device(id, device, local_rank + rank_offset, global_world_size)?;
    ...
    let all_visible_devices = device_map::get_all_similar_devices(&available_devices[0])?;
    let mapper = DeviceMapSetting::Nccl {
        nm_device: device_map::resolve_nm_device_for_visible_devices(
            &available_devices[0],
            &all_visible_devices,
        ),
        comm: Arc::new(comm),

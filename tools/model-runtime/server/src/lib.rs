//! > **engine.rs server core**
//!
//! ## About
//!
//! This crate powers engine.rs server. It exposes the underlying functionality
//! allowing others to implement and extend the server implementation.
//!
//! ### Features
//! 1. Incorporate engine.rs server into another axum.rs project.
//! 2. Hook into the engine.rs server lifecycle.
//!
//! ### Example
//! ```no_run
//! use std::sync::Arc;
...
//!     engine_for_server_builder::MistralRsForServerBuilder,
//!     engine_server_router_builder::MistralRsServerRouterBuilder,
...
//! #[derive(Clone)]
//! pub struct AppState {
//!     pub engine_state: SharedMistralRsState,
//!     pub db_create: fn(),
//! }
//!
//! #[tokio::main]
...
//!         .unwrap();
//!
//!     let engine_base_path = "/api/engine";
//!
//!     let engine_routes = MistralRsServerRouterBuilder::new()
//!         .with_engine(shared_engine.clone())
//!         .with_include_swagger_routes(false)
//!         .with_base_path(engine_base_path)
//!         .build()
//!         .await
//!         .unwrap();
//!
//!     let engine_doc = get_openapi_doc(Some(engine_base_path));
//!     let mut api_docs = ApiDoc::openapi();
//!     api_docs.merge(engine_doc);
//!
//!     let app_state = Arc::new(AppState {
//!         engine_state: shared_engine,
...
//!         .nest(engine_base_path, engine_routes)
...
//!         match parse_request(oai_request, engine_state.clone(), tx).await {
//!             Ok(x) => x,
//!             Err(e) => return handle_error(engine_state, e.into()),
//!         };
//!
//!     dbg!(request.clone());
//!
//!     if let Err(e) = send_request(&engine_state, request).await {
//!         return handle_error(engine_state, e.into());
//!     }
//!
//!     if is_streaming {
...
//!             });
//!
//!         let streamer = create_streamer(rx, engine_state.clone(), Some(on_chunk), Some(on_done));
//!
//!         ChatCompletionResponder::Sse(streamer)
//!     } else {
//!         let response = process_non_streaming_response(&mut rx, engine_state.clone()).await;
//!
//!         match &response {
...
pub mod completions;
pub mod embeddings;
#[path = "mistralrs_for_server_builder.rs"]
pub mod engine_for_server_builder;
#[path = "mistralrs_server_router_builder.rs"]
pub mod engine_server_router_builder;

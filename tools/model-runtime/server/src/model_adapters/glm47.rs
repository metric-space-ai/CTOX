...
pub fn matches(model_id: &str) -> bool {
    ...
    lowered.contains("glm-4.7") || lowered.contains("glm_4.7") || lowered.contains("glm4.7")
}
...
pub fn parse_response_items(raw_text: &str) -> Vec<AdaptedResponseItem> {
    let (reasoning_text, visible_text) = split_glm_reasoning_and_content(raw_text);
    ...
    let plain_text = plain_text.replace("<|observation|>", "");
...
fn split_glm_reasoning_and_content(text: &str) -> (Option<String>, String) {

...
use crate::{
    ...
    model_adapters::ResponsesModelAdapter,
    openai::{
...
///
/// ```no_run
/// use engine_server_core::chat_completion::ChatCompletionOnChunkCallback;
///
...
///
/// ```no_run
/// use engine_server_core::chat_completion::ChatCompletionOnDoneCallback;
///
...
pub async fn parse_request(
    ...
    let stop_toks = convert_stop_tokens(
        oairequest.stop_seqs,
        oairequest
            .stop_token_ids
            .or_else(|| adapter.chat_stop_token_ids()),
    );
...
mod tests {
    ...
    use engine_core::ReasoningEffort;

    #[test]
    fn parse_reasoning_effort_preserves_none() {
        assert_eq!(
            parse_reasoning_effort(&Some("none".to_string())),
            Some(ReasoningEffort::None)
        );
        assert_eq!(
            parse_reasoning_effort(&Some("low".to_string())),

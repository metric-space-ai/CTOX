...
use engine_core::{DrySamplingParams, MistralRs, StopTokens as InternalStopTokens};

use crate::{
    openai::{StopTokenIds, StopTokens},
...
/// Helper function to convert from the OpenAI stop tokens to the engine.rs
/// internal stop tokens.
pub(crate) fn convert_stop_tokens(
    stop_seqs: Option<StopTokens>,
    stop_token_ids: Option<StopTokenIds>,
) -> Option<InternalStopTokens> {
    if let Some(stop_token_ids) = stop_token_ids {
        return match stop_token_ids {
            StopTokenIds::Multi(ids) => Some(InternalStopTokens::Ids(ids)),
            StopTokenIds::Single(id) => Some(InternalStopTokens::Ids(vec![id])),
        };
    }

...
pub struct LocalEmbeddingsRequest {
    pub model: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LocalEmbeddingsResponse {
    pub model: String,
...
pub async fn create_local_embeddings(
    state: SharedMistralRsState,
    request: LocalEmbeddingsRequest,
) -> Result<LocalEmbeddingsResponse> {
    if let Err(err) = validate_model_name(&request.model, state.clone()) {
        return Err(err);
    }
    if request.inputs.is_empty() {
        anyhow::bail!("input must contain at least one entry.");
    }
    ...
    let futures = request.inputs.into_iter().map(|prompt| {
        let state = state.clone();
        let model_override = model_override.clone();
        async move {
            fetch_embedding(
    ...
    for result in results {
        let item = result?;
        total_prompt_tokens = total_prompt_tokens.saturating_add(item.prompt_tokens);
        total_tokens = total_tokens.saturating_add(item.total_tokens);
        data.push(item.embedding);
    }

    Ok(LocalEmbeddingsResponse {
        model: request.model,
        data,
        prompt_tokens: saturating_to_u32(total_prompt_tokens),
        total_tokens: saturating_to_u32(total_tokens),

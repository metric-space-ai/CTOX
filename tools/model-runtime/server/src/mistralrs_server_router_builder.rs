//! ## engine.rs server router builder.
...
/// A builder for creating a engine.rs server router with configurable options.
///
/// ### Examples
///
/// Basic usage:
/// ```ignore
/// use engine_server_core::engine_server_router_builder::MistralRsServerRouterBuilder;
///
/// let router = MistralRsServerRouterBuilder::new()
///     .with_engine(engine_instance)
///     .build()
///     .await?;
/// ```
///
...
///     .with_base_path("/api/engine")
///     .build()
///     .await?;
/// ```
pub struct MistralRsServerRouterBuilder {
    /// The shared engine.rs instance
    engine: Option<SharedMistralRsState>,
...
impl MistralRsServerRouterBuilder {
    ...
    /// Sets the shared engine.rs instance
    pub fn with_engine(mut self, engine: SharedMistralRsState) -> Self {
    ...
    /// useful when including the engine.rs server instance in another axum project.
    ///
    /// Only available when the `swagger-ui` feature is enabled.
    #[cfg(feature = "swagger-ui")]
    ...
    pub async fn build(self) -> Result<Router> {
        let engine = self
            .engine
            .ok_or_else(|| anyhow::anyhow!("`engine` instance must be set. Use `with_engine`."))?;

        #[allow(unused_mut)]
        let mut router = init_router(engine, self.allowed_origins, self.max_body_limit)?;

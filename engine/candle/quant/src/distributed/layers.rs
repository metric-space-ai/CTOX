...
impl QuantMethod for RowParallelLayer {
    ...
    fn forward(&self, a: &Tensor) -> Result<Tensor> {
        let (weight_dtype, weight_device) = self.weight.dtype_and_device();
        if !a.device().same_device(&weight_device) {
            candle_core::bail!(
                "row_parallel.forward input/weight device mismatch: input={:?} weight={:?} input_shape={:?} weight_dtype={:?}",
                a.device().location(),
        ...
        if let Some(bias) = &self.bias {
            if !xs.device().same_device(bias.device()) {
                candle_core::bail!(
                    "row_parallel.forward bias device mismatch: output={:?} bias={:?} output_shape={:?} bias_shape={:?}",
                    xs.device().location(),
...
impl QuantMethod for ColumnParallelLayer {
    ...
    fn forward(&self, a: &Tensor) -> Result<Tensor> {
        ...
        if !a.device().same_device(&weight_device) {
            candle_core::bail!(
                "column_parallel.forward input/weight device mismatch: input={:?} weight={:?} input_shape={:?} weight_dtype={:?}",
                a.device().location(),
                weight_device.location(),
                a.shape().dims(),
                weight_dtype
        ...
        if let Some(bias) = &self.bias {
            if !xs.device().same_device(bias.device()) {
                candle_core::bail!(
                    "column_parallel.forward bias device mismatch: output={:?} bias={:?} output_shape={:?} bias_shape={:?}",
                    xs.device().location(),
                    bias.device().location(),
                    xs.shape().dims(),
                    bias.shape().dims()

...
use mistralrs_paged_attn::{kv_scale_update, paged_attention, reshape_and_cache, turbo_rotate};
...
fn turboquant_debug_enabled() -> bool {
    std::env::var_os("MISTRALRS_TURBOQUANT_DEBUG").is_some()
}

fn turboquant_native_paged_enabled() -> bool {
    std::env::var_os("MISTRALRS_TURBOQUANT_DISABLE_NATIVE_PAGED").is_none()
}

fn turboquant_rotation_enabled() -> bool {
    std::env::var_os("MISTRALRS_TURBOQUANT_DISABLE_ROTATION").is_none()
}
...
fn turboquant_debug_log(message: impl AsRef<str>) {
    if turboquant_debug_enabled() {
        tracing::info!(
            target: "mistralrs_core::paged_attention::turboquant_debug",
            "{}",
            message.as_ref()
        );
    }
...
impl PagedAttention {
    ...
    pub fn forward(
        ...
        let turbo_bits = if turboquant_cache {
            Some(turboquant_bits_for_cache(
        ...
        if turboquant_cache {
            ...
            .map_err(|e| {
                candle_core::Error::Msg(format!(
                    "turboquant encode/write failed (seq_len={}, kv_heads={}, head_size={}): {e}",
                    seq_len, key_value_heads, head_size
                ))
            })?;
            ...
            let turbo_decode_via_native_paged = turbo_bits
                .is_some_and(|bits| turboquant_native_rotated_cuda_supported(bits, head_size))
                && turbo_rotated
            ...
            if !turbo_decode_via_native_paged {
                ...
                .map_err(|e| {
                    candle_core::Error::Msg(format!(
                        "turboquant gather/read failed (seq_len={}, kv_heads={}, head_size={}): {e}",
                        seq_len, key_value_heads, head_size
                    ))
                })?;
                ...
                let attn_out = Sdpa
                    ...
                    .map_err(|e| {
                        candle_core::Error::Msg(format!(
                            "turboquant attention failed (seq_len={}, kv_heads={}, head_size={}, max_k={}, cu_kv_len={}): {e}",
                            seq_len,
                            key_value_heads,
                            head_size,
                            max_k,
...
fn turboquant_bits_for_cache(
    ...
    for bits in [
        ...
        if key_payload_bytes == bits.packed_key_bytes_for_dim(head_dim)
            && value_payload_bytes == bits.packed_value_bytes_for_dim(head_dim)
        {
            return Ok(bits);
        }
    }
    candle_core::bail!(
        "unable to infer turboquant cache type for head_dim={head_dim}, key_payload_bytes={key_payload_bytes}, value_payload_bytes={value_payload_bytes}"
    );
}
...
fn turboquant_native_rotated_cuda_supported(bits: TurboQuantBits, head_dim: usize) -> bool {
    bits == TurboQuantBits::Three && head_dim % 128 == 0
}
...
fn turboquant_reshape_and_cache(
    ...
    let bits = turboquant_bits_for_cache(key_cache, value_cache, head_dim)?;
    ...
    for (token_idx, slot) in slots.into_iter().enumerate() {
        ...
        if bits == TurboQuantBits::Three && turbo_rotated {
            ...
            let flat_value = values[token_idx]
                ...
                .flat_map(|head| head.iter().copied())
                .collect::<Vec<_>>();
            let compressed_key = flat_artifacts.compress_key_with_rotation(&flat_key, turbo_rotated);
            ...
            if token_idx == 0 {
                let key_recon = if turbo_rotated {
                    flat_artifacts.reconstruct_key_mse_rotated_with_rotation(
                        &compressed_key,
                        turbo_rotated,
                    )
                } else {
                ...
                let value_recon = if turbo_rotated {
                    flat_artifacts.decompress_value_rotated_with_rotation(
                        &compressed_value,
                        turbo_rotated,
                    )
                } else {
            ...
            for head in 0..num_heads {
                ...
                let compressed_value = artifacts
                    .compress_value_with_rotation(&values[token_idx][head], turbo_rotated);
...
fn turboquant_gather_kv_cache(
        ...
        (paged_kv_indptr, paged_kv_indices, paged_kv_last_page_len)
    {
            ...
            && turboquant_native_rotated_cuda_supported(bits, head_dim)
        {
            ...
            let (k, v) = mistralrs_paged_attn::gather_kv_cache(
                key_cache,
                value_cache,
                None,
                None,
...
#[cfg(test)]
mod tests {
    use super::turboquant_should_use_windowed_paged_kv_metadata;

...
pub(crate) struct TekkenTextEncoder {
    pattern: FancyRegex,
    mergeable_ranks: AHashMap<Vec<u8>, u32>,
    num_special_tokens: u32,
}

impl TekkenTextEncoder {
    pub(crate) fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let raw = std::fs::read(path)?;
        let tekken: TekkenTokenizer = serde_json::from_slice(&raw)?;
        let pattern = FancyRegex::new(&tekken.config.pattern)
            .map_err(|err| anyhow!("failed to compile Tekken regex: {err}"))?;
        let mut mergeable_ranks = AHashMap::with_capacity(tekken.vocab.len());
        for (expected_rank, entry) in tekken.vocab.iter().enumerate() {
            if entry.rank != expected_rank {
                anyhow::bail!(
                    "invalid Tekken vocab rank ordering: expected {expected_rank}, got {}",
                    entry.rank
                );
            }
            let token_bytes =
                base64::engine::general_purpose::STANDARD.decode(&entry.token_bytes)?;
            #[allow(clippy::cast_possible_truncation)]
            mergeable_ranks.insert(token_bytes, entry.rank as u32);
        }
        Ok(Self {
            pattern,
            mergeable_ranks,
            #[allow(clippy::cast_possible_truncation)]
            num_special_tokens: tekken.config.default_num_special_tokens as u32,
        })
    }

    pub(crate) fn encode_ordinary(&self, text: &str) -> Result<Vec<u32>> {
        if text.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for matched in self.pattern.find_iter(text) {
            let matched = matched
                .map_err(|err| anyhow!("failed to evaluate Tekken regex against prompt: {err}"))?;
            if matched.start() > covered_until {
                tokens.extend(self.encode_piece(&text.as_bytes()[covered_until..matched.start()])?);
            }
            tokens.extend(self.encode_piece(matched.as_str().as_bytes())?);
            covered_until = matched.end();
        }
        if covered_until < text.len() {
            tokens.extend(self.encode_piece(&text.as_bytes()[covered_until..])?);
        }
        Ok(tokens
            .into_iter()
            .map(|token| token + self.num_special_tokens)
            .collect())
    }

    fn encode_piece(&self, piece: &[u8]) -> Result<Vec<u32>> {
        if piece.is_empty() {
            return Ok(Vec::new());
        }
        if let Some(rank) = self.mergeable_ranks.get(piece) {
            return Ok(vec![*rank]);
        }

        let mut parts = piece.iter().map(|byte| vec![*byte]).collect::<Vec<_>>();
        while parts.len() > 1 {
            let mut best_merge = None;
            for idx in 0..parts.len() - 1 {
                let mut merged = Vec::with_capacity(parts[idx].len() + parts[idx + 1].len());
                merged.extend_from_slice(&parts[idx]);
                merged.extend_from_slice(&parts[idx + 1]);
                if let Some(rank) = self.mergeable_ranks.get(&merged) {
                    match best_merge {
                        Some((_, best_rank)) if *rank >= best_rank => {}
                        _ => best_merge = Some((idx, *rank)),
                    }
                }
            }

            let Some((merge_idx, _)) = best_merge else {
                break;
            };
            let rhs = parts.remove(merge_idx + 1);
            parts[merge_idx].extend_from_slice(&rhs);
        }

        parts
            .into_iter()
            .map(|part| {
                self.mergeable_ranks.get(&part).copied().ok_or_else(|| {
                    anyhow!(
                        "Tekken tokenizer is missing a mergeable rank for byte sequence of len {}",
                        part.len()
                    )
                })
            })

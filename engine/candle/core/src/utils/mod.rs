...
pub fn format_error_chain(error: &(dyn std::error::Error + 'static)) -> String {
    let mut lines = vec![format!("0: {error}")];
    let mut current = error.source();
    let mut index = 1usize;
    while let Some(source) = current {
        lines.push(format!("{index}: {source}"));
    ...
    ($stage: tt, $fallible:expr, $seq_slice:expr, $pipeline:expr, $label:tt, $prefix_cacher:expr) => {
        match $fallible {
            ...
            Err(e) => {
                ...
                use tracing::error;
                error!(
                    "{} - Model failed with error chain v2:\n{}",
                    $stage,
                    $crate::utils::format_error_chain(&e)
                );
                error!("{} - Model failed with debug: {:?}", $stage, &e);

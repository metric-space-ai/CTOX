...
impl AutoLoader {
    ...
    fn read_config_from_hf(
        ...
        let contents = match Self::try_get_file(&api, model_id, "config.json") {
            ...
            Ok(None) => match Self::try_get_file(&api, model_id, "params.json") {
                Ok(Some(path)) => Some(std::fs::read_to_string(&path)?),
                Ok(None) => None,
                Err(err) => {
                    let issue = remote_issue_from_api_error(model_id, Some("params.json"), &err);
                    warn!(
                        "Auto loader could not fetch `params.json` for `{}`: {}",
            ...
            Err(err) => {
                ...
                remote_access_issue = Some(issue);
                match Self::try_get_file(&api, model_id, "params.json") {
                    Ok(Some(path)) => Some(std::fs::read_to_string(&path)?),
                    Ok(None) => None,
                    Err(_) => None,
                }
    ...
    fn detect(&self, artifacts: &ConfigArtifacts) -> Result<Detected> {
        ...
        if artifacts.contents.is_none() && artifacts.repo_files.iter().any(|f| f == "params.json") {
            info!("Detected `params.json` in repo without readable contents; routing as Voxtral vision fallback.");

...
use super::{DeviceMappedModelLoader, QuantizationConfigShim};

const GPU_RESERVE_FRACTION: f64 = 0.02;
const GPU_MIN_RESERVE_BYTES: usize = 512 * 1024 * 1024; // 512MB safety buffer

fn requires_accelerator_only_layers(config: &str) -> bool {
    serde_json::from_str::<QuantizationConfigShim>(config)
        .ok()
        .and_then(|shim| shim.quantization_config)
        .is_some_and(|quant| matches!(quant, QuantizedConfig::MXFP4 {}))
}
...
pub fn get_device_layers(
    ...
    if !has_unified_memory && !requires_accelerator_only_layers(config) {
    ...
    info!(
        "Auto device map sizing: total_model={}MB non_mapped={}MB mapped_layers={}MB mapped_max={}MB non_mapped_max={}MB kv_cache={}MB accelerator_only={}",
        b_to_mb!(total_model_size_in_bytes),
        b_to_mb!(non_mapped_size_in_bytes),
        b_to_mb!(remaining),
        b_to_mb!(mapped_max),
        b_to_mb!(non_mapped_max),
        b_to_mb!(kv_cache_bytes),
        requires_accelerator_only_layers(config)
    );
    for (avail_bytes, dev) in &avail {
        info!(
            "Auto device candidate: {} available={}MB capped={}MB",
            dev.device_pretty_repr(),
            b_to_mb!(*avail_bytes),
            b_to_mb!(device_cap(*avail_bytes, dev))
        );
    }
    ...
    while remaining > 0 && !avail.is_empty() {
        ...
        let layers_on_dev = if cap >= required_whole_capacity {
            ...
            if count > 0 {
                ...
                warn!(
                    "Device {} can fit 0 layers. cap={}MB used_base={}MB remaining_weights={}MB next_layer_plus_kv={}MB params={params}",
                    dev.device_pretty_repr(),
                    b_to_mb!(cap),
                    b_to_mb!(used),
                    b_to_mb!(remaining),
                    layer_sizes_in_bytes
                        .last()
                        .map(|next| b_to_mb!(next + kv_cache_bytes))
                        .unwrap_or(0),
                );
                ordinal += 1;
                continue;

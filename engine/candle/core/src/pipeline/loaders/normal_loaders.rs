...
impl NormalLoaderType {
    pub fn from_causal_lm_name(name: &str) -> Result<Self> {
        match name {
            ...
            "NemotronHForCausalLM" => Ok(Self::NemotronH),
...
impl FromStr for NormalLoaderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            a => Err(format!("Unknown architecture `{a}`. Possible architectures: `mistral`, `gemma`, `mixtral`, `llama`, `phi2`, `phi3`, `qwen2`, `gemma2`, `starcoder2`, `phi3.5moe`, `deepseekv2`, `deepseekv3`, `qwen3`, `glm4`, `glm4moelite`, `glm4moe`, `qwen3moe`, `smollm3`, `granitemoehybrid`, `gpt_oss`, `nemotron_h`, `qwen3next`.")),
        }
    }
}
...
impl Display for NormalLoaderType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NemotronH => write!(f, "nemotron_h"),
...
impl AutoNormalLoader {
    fn get_loader(config: &str) -> Result<Box<dyn NormalModelLoader>> {
        ...
        match tp {
            ...
            NormalLoaderType::NemotronH => Ok(Box::new(NemotronHLoader)),
...
/// [`NormalLoader`] for NemotronH models such as Nemotron-Cascade.
pub struct NemotronHLoader;

impl NormalModelLoader for NemotronHLoader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        let cfg: crate::models::nemotron_h::Config = serde_json::from_str(config)?;

        Ok(Box::new(crate::models::nemotron_h::Model::new(
            &cfg,
            vb,
            self.is_gptx(config)?,
            normal_loading_metadata,
    ...
    fn load_xlora(
        ...
        anyhow::bail!("NemotronH does not support X-LoRA")
    }
...
impl IsqModelLoader for NemotronHLoader {
    fn isq_layer_regexes(&self, _config: &str) -> Result<Vec<Regex>> {
        Ok(vec![
            Regex::new(r"lm_head\.(weight|bias)$")?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.q_proj\.(weight|bias)$")?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.k_proj\.(weight|bias)$")?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.v_proj\.(weight|bias)$")?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.o_proj\.(weight|bias)$")?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.shared_experts\.up_proj\.(weight|bias)$")?,
            Regex::new(
                r"backbone\.layers\.(\d+)\.mixer\.shared_experts\.down_proj\.(weight|bias)$",
            )?,
            Regex::new(r"backbone\.layers\.(\d+)\.mixer\.experts\.(\d+)\.up_proj\.(weight|bias)$")?,
            Regex::new(
                r"backbone\.layers\.(\d+)\.mixer\.experts\.(\d+)\.down_proj\.(weight|bias)$",
            )?,
        ])
    }
...
impl DeviceMappedModelLoader for NemotronHLoader {
    fn mapped_max_act_size_elems(
        &self,
        config: &str,
        params: &AutoDeviceMapParams,
    ...
    fn layer_sizes_in_bytes(
        ...
        let item_size = dtype.size_in_bytes() / weight_pack_factor.max(1);
        let mut sizes = Vec::with_capacity(layer_types.len());
        for layer_type in layer_types {
            let size = match layer_type {
                crate::models::nemotron_h::NemotronLayerType::Attention => {
                    let q = cfg.hidden_size * cfg.num_attention_heads * cfg.head_dim;
                    let kv = cfg.hidden_size * cfg.num_key_value_heads() * cfg.head_dim * 2;
                    let o = cfg.hidden_size * cfg.num_attention_heads * cfg.head_dim;
                    let norm = cfg.hidden_size;
                    (q + kv + o + norm) * item_size
                }
                crate::models::nemotron_h::NemotronLayerType::Mamba => {
                    let in_proj = cfg.hidden_size
                        * (cfg.mamba_intermediate_size()
                            + cfg.mamba_conv_dim()
                            + cfg.mamba_num_heads);
                    let conv = cfg.mamba_conv_dim() * cfg.conv_kernel;
                    let out_proj = cfg.hidden_size * cfg.mamba_intermediate_size();
                    let misc =
                        cfg.mamba_num_heads * 2 + cfg.hidden_size + cfg.mamba_intermediate_size();
                    (in_proj + conv + out_proj + misc) * item_size
                }
                crate::models::nemotron_h::NemotronLayerType::Moe => {
                    let routed = cfg.n_routed_experts
                        * (cfg.hidden_size * cfg.moe_intermediate_size
                            + cfg.moe_intermediate_size * cfg.hidden_size);
                    let shared = cfg.hidden_size * cfg.moe_shared_expert_intermediate_size
                        + cfg.moe_shared_expert_intermediate_size * cfg.hidden_size;
                    let gate = cfg.n_routed_experts * cfg.hidden_size + cfg.n_routed_experts;
                    (routed + shared + gate + cfg.hidden_size) * item_size
                }
            };
            sizes.push(size);
        }
    ...
    fn non_mapped_size_in_bytes(
        ...
        let cfg: crate::models::nemotron_h::Config = serde_json::from_str(config)?;
        let item_size = dtype.size_in_bytes() / weight_pack_factor.max(1);
        Ok((cfg.vocab_size * cfg.hidden_size * 2 + cfg.hidden_size) * item_size)
    }

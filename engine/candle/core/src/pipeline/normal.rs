...
const MISTRALRS_PAGED_ATTN_CACHE_TYPE: &str = "MISTRALRS_PAGED_ATTN_CACHE_TYPE";

fn paged_cache_type_env_value(cache_type: crate::paged_attention::PagedCacheType) -> &'static str {
    match cache_type {
        crate::paged_attention::PagedCacheType::Auto => "auto",
        crate::paged_attention::PagedCacheType::F8E4M3 => "f8e4m3",
        crate::paged_attention::PagedCacheType::TurboQuant2 => "turboquant2",
        crate::paged_attention::PagedCacheType::TurboQuant3 => "turboquant3",
        crate::paged_attention::PagedCacheType::TurboQuant4 => "turboquant4",
    }
}

fn set_paged_attn_cache_type_env(config: Option<PagedAttentionConfig>) {
    if let Some(config) = config {
        std::env::set_var(
            MISTRALRS_PAGED_ATTN_CACHE_TYPE,
            paged_cache_type_env_value(config.cache_type),
        );
    } else {
...
impl Loader for NormalLoader {
    ...
    fn load_model_from_path(
        ...
        set_paged_attn_cache_type_env(paged_attn_config);

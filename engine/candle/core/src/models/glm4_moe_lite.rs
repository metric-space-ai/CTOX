...
fn glm_uses_quantized_paged_cache() -> bool {
    matches!(
        std::env::var("MISTRALRS_PAGED_ATTN_CACHE_TYPE")
            .ok()
            .as_deref(),
        Some("f8e4m3" | "turboquant2" | "turboquant3" | "turboquant4")
    )
}

fn glm_can_use_mla_paged_kv(attention_mechanism: AttentionImplementation, device: &Device) -> bool {
    matches!(attention_mechanism, AttentionImplementation::PagedAttention)
        ...
        && !glm_uses_quantized_paged_cache()
}
...
impl Attention {
    ...
    fn new(
        ...
        let use_mla_paged_kv = paged_attn.is_some() && !glm_uses_quantized_paged_cache();
...
impl Glm4MoeLite {
    pub fn new(
        ...
        let use_mla_paged_kv =
            glm_can_use_mla_paged_kv(attention_mechanism, &normal_loading_metadata.real_device);
...
mod tests {
    ...
    fn glm_treats_fp8_cache_as_quantized() {
        unsafe {
            std::env::set_var("MISTRALRS_PAGED_ATTN_CACHE_TYPE", "f8e4m3");
        }
        assert!(glm_uses_quantized_paged_cache());
        unsafe {
            std::env::remove_var("MISTRALRS_PAGED_ATTN_CACHE_TYPE");
        }
    }
    ...
    fn glm_leaves_auto_cache_unquantized() {
        unsafe {
            std::env::set_var("MISTRALRS_PAGED_ATTN_CACHE_TYPE", "auto");
        }
        assert!(!glm_uses_quantized_paged_cache());
        unsafe {
            std::env::remove_var("MISTRALRS_PAGED_ATTN_CACHE_TYPE");

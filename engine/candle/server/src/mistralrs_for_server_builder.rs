...
async fn resolve_pending_isq_layers_for_pipeline(pipeline: &LoadedPipeline) -> Result<()> {
    let should_resolve = std::env::var("MISTRALRS_RESOLVE_PENDING_ISQ_AT_STARTUP")
        .ok()
        .map(|raw| {
    ...
    if !should_resolve {
        ...
        warn!(
            "Skipping eager pending immediate-ISQ resolution for `{pipeline_name}` due to MISTRALRS_RESOLVE_PENDING_ISQ_AT_STARTUP."
        );
        return Ok(());
    }

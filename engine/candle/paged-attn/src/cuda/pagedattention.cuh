...
__device__ __constant__ float kTurbo3CentroidsPaged[8] = {
    -0.190685f, -0.117832f, -0.065717f, -0.021460f,
     0.021460f,  0.065717f,  0.117832f,  0.190685f,
};

template <typename scalar_t, int VEC_SIZE>
__device__ __forceinline__ typename Vec<scalar_t, VEC_SIZE>::Type
make_vec_from_floats(const float* vals) {
  using VecT = typename Vec<scalar_t, VEC_SIZE>::Type;
  VecT out;
  if constexpr (VEC_SIZE == 1) {
    ...
    if constexpr (std::is_same_v<VecT, float4>) {
      ...
      Float4_ tmp;
      tmp.x = make_float2(vals[0], vals[1]);
      tmp.y = make_float2(vals[2], vals[3]);
      from_float(out, tmp);
    }
    ...
    tmp.x = make_float2(vals[0], vals[1]);
    tmp.y = make_float2(vals[2], vals[3]);
    tmp.z = make_float2(vals[4], vals[5]);
    tmp.w = make_float2(vals[6], vals[7]);
    from_float(out, tmp);
  }
  return out;
}

__device__ __forceinline__ float turbo3_half_to_float_paged(const uint8_t lo,
                                                            const uint8_t hi) {
  const uint16_t bits =
...
__device__ __forceinline__ float turbo3_load_scalar(
    const uint8_t* cache_head_base,
    const int block_size,
    const int slot,
    const int dim) {
  const int group_idx = dim / TURBO3_GROUP_SIZE;
  const int dim_in_group = dim % TURBO3_GROUP_SIZE;
  const int block_idx = dim_in_group / TURBO3_BLOCK_SIZE;
  const int dim_in_block = dim_in_group % TURBO3_BLOCK_SIZE;
  const int group_byte_offset = group_idx * TURBO3_GROUP_BYTES;
  const int block_byte_offset = group_byte_offset + block_idx * TURBO3_BLOCK_BYTES;

  const uint8_t norm_lo = cache_head_base[(block_byte_offset + 0) * block_size + slot];
  const uint8_t norm_hi = cache_head_base[(block_byte_offset + 1) * block_size + slot];
  const float norm = turbo3_half_to_float_paged(norm_lo, norm_hi);

  const int low_byte = block_byte_offset + 2 + (dim_in_block / 4);
  const int sign_byte = block_byte_offset + 10 + (dim_in_block / 8);
  const uint8_t low =
      (cache_head_base[low_byte * block_size + slot] >> ((dim_in_block % 4) * 2)) & 0x3;
  const uint8_t hi =
      (cache_head_base[sign_byte * block_size + slot] >> (dim_in_block % 8)) & 0x1;
  return kTurbo3CentroidsPaged[low | (hi << 2)] * norm;
}

template <int N>
__device__ __forceinline__ void turbo3_load_key_vec(
    const uint8_t* cache_head_base,
    const int block_size,
    const int slot,
    const int dim_start,
    float* vals) {
  static_assert(N == 1 || N == 2 || N == 4 || N == 8, "unsupported turbo3 key vec size");

  const int group_idx = dim_start / TURBO3_GROUP_SIZE;
  const int dim_in_group = dim_start % TURBO3_GROUP_SIZE;
  const int block_idx = dim_in_group / TURBO3_BLOCK_SIZE;
  const int dim_in_block = dim_in_group % TURBO3_BLOCK_SIZE;
  const int group_byte_offset = group_idx * TURBO3_GROUP_BYTES;
  const int block_byte_offset = group_byte_offset + block_idx * TURBO3_BLOCK_BYTES;

  const uint8_t norm_lo =
      cache_head_base[(block_byte_offset + 0) * block_size + slot];
  const uint8_t norm_hi =
      cache_head_base[(block_byte_offset + 1) * block_size + slot];
  const float norm = turbo3_half_to_float_paged(norm_lo, norm_hi);
  float cn[8];
#pragma unroll
  for (int c = 0; c < 8; ++c) {
    cn[c] = kTurbo3CentroidsPaged[c] * norm;
  }

  const int low_byte_base = block_byte_offset + 2 + (dim_in_block / 4);
  const int sign_byte_base = block_byte_offset + 10 + (dim_in_block / 8);
  const uint8_t qs_lo = cache_head_base[low_byte_base * block_size + slot];
  const uint8_t qs_hi =
      (N > 4 || (dim_in_block % 4) + N > 4)
          ? cache_head_base[(low_byte_base + 1) * block_size + slot]
          : 0;
  uint16_t signs = cache_head_base[sign_byte_base * block_size + slot];
  if ((dim_in_block % 8) + N > 8) {
    signs |= static_cast<uint16_t>(
                 cache_head_base[(sign_byte_base + 1) * block_size + slot])
             << 8;
  }

#pragma unroll
  for (int i = 0; i < N; ++i) {
    const int rel = dim_in_block + i;
    const int rel4 = rel % 4;
    const int rel8 = rel % 8;
    const uint8_t qs_b = ((dim_in_block % 4) + i < 4) ? qs_lo : qs_hi;
    const uint8_t low2 = (qs_b >> (rel4 * 2)) & 0x3;
    const uint8_t hi1 = (signs >> rel8) & 0x1;
    vals[i] = cn[low2 | (hi1 << 2)];
  }
}

template <int N>
__device__ __forceinline__ void turbo3_load_value_vec(
    const uint8_t* cache_head_base,
    const int block_size,
    const int slot_start,
    const int dim,
    float* vals) {
  static_assert(N == 1 || N == 2 || N == 4 || N == 8, "unsupported turbo3 value vec size");
  ...
  const int low_byte = block_byte_offset + 2 + (dim_in_block / 4);
  const int sign_byte = block_byte_offset + 10 + (dim_in_block / 8);
  const int low_shift = (dim_in_block % 4) * 2;
  const int sign_shift = dim_in_block % 8;

#pragma unroll
  for (int i = 0; i < N; ++i) {
    const int slot = slot_start + i;
    const uint8_t norm_lo =
        cache_head_base[(block_byte_offset + 0) * block_size + slot];
    const uint8_t norm_hi =
        cache_head_base[(block_byte_offset + 1) * block_size + slot];
    const float norm = turbo3_half_to_float_paged(norm_lo, norm_hi);
    const uint8_t low =
        (cache_head_base[low_byte * block_size + slot] >> low_shift) & 0x3;
    const uint8_t hi =
        (cache_head_base[sign_byte * block_size + slot] >> sign_shift) & 0x1;
    vals[i] = kTurbo3CentroidsPaged[low | (hi << 2)] * norm;
  }
}
...
#pragma unroll
  ...
  constexpr int x = kv_dt == vllm::Fp8KVCacheDataType::kTurboQuant3
                        ? 1
                        : 16 / sizeof(cache_t);
...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        if constexpr (kv_dt == vllm::Fp8KVCacheDataType::kTurboQuant3) {
          float vals[VEC_SIZE];
          const uint8_t* k_head_ptr =
              reinterpret_cast<const uint8_t*>(k_cache) +
              physical_block_number * kv_block_stride +
              kv_head_idx * kv_head_stride;
          turbo3_load_key_vec<VEC_SIZE>(
              k_head_ptr, BLOCK_SIZE, physical_block_offset,
              vec_idx * VEC_SIZE, vals);
          k_vecs[j] = make_vec_from_floats<scalar_t, VEC_SIZE>(vals);
        } else if constexpr (kv_dt == vllm::Fp8KVCacheDataType::kAuto) {
    ...
    for (int i = 0; i < NUM_ROWS_PER_THREAD; i++) {
      ...
      if (row_idx < HEAD_SIZE) {
        ...
        if constexpr (kv_dt == vllm::Fp8KVCacheDataType::kTurboQuant3) {
          ...
          const uint8_t* v_head_ptr =
              reinterpret_cast<const uint8_t*>(v_cache) +
              physical_block_number * kv_block_stride +
              kv_head_idx * kv_head_stride;
          turbo3_load_value_vec<V_VEC_SIZE>(
              v_head_ptr, BLOCK_SIZE, physical_block_offset, row_idx, vals);
          v_vec = make_vec_from_floats<scalar_t, V_VEC_SIZE>(vals);
        } else if constexpr (kv_dt == vllm::Fp8KVCacheDataType::kAuto) {

...
use mistralrs_quant::{
    Convolution, QuantizedConfig, ShardedVarBuilder,
};
use std::sync::Mutex;

use crate::{
    layers::{Activation, repeat_kv},
...
impl MimiConv1d {
    ...
    fn extra_padding(&self, len: usize) -> usize {
        let n_frames =
            (len as f64 - self.effective_kernel_size as f64 + self.padding_total as f64)
                / self.stride as f64
                + 1.0;
        ...
        let padded = match padding_cache {
            ...
            None => {
                ...
                match self.pad_mode.as_str() {
                    ...
                    other => {
                        candle_core::bail!(
                            "Unsupported Qwen3-TTS MimiConv1d pad_mode `{other}`."
                        );
                    }
                }
            }
...
impl MimiEncoderLayer {
    ...
    fn append_cache_specs(&self, out: &mut Vec<(usize, String, usize)>) {
        match self {
            Self::Conv(layer) => out.push((layer.padding_total, layer.pad_mode.clone(), layer.in_channels)),
...
impl MimiTransformer {
    ...
    fn forward(&self, xs: &Tensor) -> candle_core::Result<Tensor> {
        ...
        for (idx, layer) in self.layers.iter().enumerate() {
            ...
            tracing::info!(
                "{}",
                tensor_debug_summary(&format!("Qwen3-TTS encoder transformer layer {idx}"), &hidden)?
            );
...
impl MimiEuclideanCodebook {
    fn new(cfg: &Qwen3TtsTokenizerEncoderConfig, vb: candle_nn::VarBuilder<'_>) -> candle_core::Result<Self> {
        Ok(Self {
    ...
    fn encode(&self, hidden_states: &Tensor) -> candle_core::Result<Tensor> {
        ...
        let flat = hidden_states
            ...
            .to_dtype(DType::F32)?;
        let embed = self.embed()?.to_device(flat.device())?.to_dtype(DType::F32)?;
        let diff = flat
            .unsqueeze(1)?
            .broadcast_sub(&embed.unsqueeze(0)?)?;
    ...
    fn topk_for_vector(
        ...
        vector: &Tensor,
        k: usize,
    ) -> candle_core::Result<Vec<(u32, f32)>> {
...
impl MimiResidualVectorQuantizer {
    ...
    fn encode(
        ...
        tracing::info!(
            ...
            tensor_debug_summary(
                &format!("Qwen3-TTS quantizer {label} input"),
                &residual,
            )?
        );
        if label == "acoustic" {
        ...
        let n_q = num_quantizers.unwrap_or(self.layers.len()).min(self.layers.len());
        ...
        for (layer_idx, layer) in self.layers.iter().take(n_q).enumerate() {
            if label == "acoustic" && layer_idx == 2 {
                let frame0_top5 = layer.codebook.topk_for_vector(&residual.i((0, .., 0))?, 5)?;
                let frame1_top5 = layer.codebook.topk_for_vector(&residual.i((0, .., 1))?, 5)?;
                tracing::info!(
            ...
            let preview = indices
                ...
                .to_vec1::<u32>()?;
            tracing::info!(
                "Qwen3-TTS quantizer {label} layer {layer_idx} codes={preview:?}"
            );
            if label == "acoustic" && layer_idx == 2 {
            ...
            if label == "acoustic" && layer_idx == 4 {
                tracing::info!(
                    "{}",
                    tensor_vec8_summary(
                        &format!("Qwen3-TTS quantizer {label} layer {layer_idx} post-residual frame0"),
                        &residual.i((0, ..8, 0))?,
                    )?
                );
                tracing::info!(
                    "{}",
                    tensor_vec8_summary(
                        &format!("Qwen3-TTS quantizer {label} layer {layer_idx} post-residual frame1"),
                        &residual.i((0, ..8, 1))?,
                    )?
                );
            }
    ...
    fn decode(&self, indices: &Tensor) -> candle_core::Result<Tensor> {
        ...
        let quantized = quantized.ok_or_else(|| {
            candle_core::Error::Msg("MimiResidualVectorQuantizer::decode got zero quantizers".into())
        })?;
...
impl Qwen3TtsTokenizerEncoder {
    pub fn new(
        ...
        let encoder_transformer =
            MimiTransformer::new(cfg, module_vb.pp("encoder_transformer"))?;
    ...
    pub fn encode_ref_codes(
        ...
        tracing::info!("{}", tensor_debug_summary("Qwen3-TTS encoder stage wave", &waveform)?);
        ...
        tracing::info!("{}", tensor_debug_summary("Qwen3-TTS encoder stage enc", &embeddings)?);
        tracing::info!(
            "{}",
            tensor_vec8_summary("Qwen3-TTS encoder stage enc frame1", &embeddings.i((0, ..8, 1))?)?
        );
        let embeddings = embeddings.transpose(1, 2)?;
        let embeddings = self.encoder_transformer.lock().unwrap().forward(&embeddings)?;
        let mut embeddings = embeddings.transpose(1, 2)?;
        tracing::info!("{}", tensor_debug_summary("Qwen3-TTS encoder stage xf", &embeddings)?);
        tracing::info!(
            "{}",
            tensor_vec8_summary("Qwen3-TTS encoder stage xf frame1", &embeddings.i((0, ..8, 1))?)?
        );
        ...
        tracing::info!("{}", tensor_debug_summary("Qwen3-TTS encoder stage down", &embeddings)?);
        tracing::info!(
            "{}",
            tensor_vec8_summary("Qwen3-TTS encoder stage down frame1", &embeddings.i((0, ..8, 1))?)?
        );
        let mut codes = self
            .quantizer

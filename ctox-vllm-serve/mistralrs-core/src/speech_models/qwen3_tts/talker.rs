...
use crate::{
    layers::{self, Activation, F32RmsNorm, RotaryEmbedding, repeat_kv},
...
fn causal_attention_mask(seq_len: usize, device: &Device) -> candle_core::Result<Tensor> {
    ...
    mask.where_cond(&zeros, &neg_inf)?.unsqueeze(0)?.unsqueeze(0)
}
...
impl SelfAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, attention_mask: Option<&Tensor>) -> candle_core::Result<Tensor> {
        ...
        let attn_probs = candle_nn::ops::softmax(
    ...
    fn forward_with_past(
        ...
        let position_offset = past_kv
            .map(|(past_k, _)| past_k.dims()[2])
            .unwrap_or(0);
        let (q, k) = self.apply_rotary(&q, &k, position_offset)?;

        let (k_cache, v_cache) = match past_kv {
            Some((past_k, past_v)) => (Tensor::cat(&[past_k, &k], 2)?, Tensor::cat(&[past_v, &v], 2)?),
...
impl TalkerDecoderLayer {
    fn new(cfg: &Qwen3TtsTalkerConfig, vb: ShardedVarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            input_layernorm: F32RmsNorm::new(cfg.hidden_size, cfg.rms_norm_eps, vb.pp("input_layernorm"))?,
    ...
    fn forward_with_past(
        ...
        let residual = hidden_states;
        let hidden_states = self.input_layernorm.forward(hidden_states)?;
        let (hidden_states, kv_cache) = self
            .self_attn
...
impl TalkerModel {
    fn new(cfg: &Qwen3TtsTalkerConfig, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            codec_embedding: layers::embedding(cfg.vocab_size, cfg.hidden_size, model_vb.pp("codec_embedding"), &None)?,
...
impl CodePredictorModel {
    fn new(cfg: &Qwen3TtsTalkerConfig, vb: ShardedVarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            codec_embeddings: (0..cp_cfg.num_code_groups - 1)
                .map(|idx| layers::embedding(cp_cfg.vocab_size, cfg.hidden_size, model_vb.pp("codec_embedding").pp(idx), &None))
    ...
    fn forward_embeds_cached(
        ...
        for (layer_idx, (layer, cache_slot)) in self.layers.iter().zip(caches.iter_mut()).enumerate() {
            let (next_hidden_states, next_cache) =
                layer.forward_with_past(&hidden_states, attention_mask.as_ref(), cache_slot.as_ref())?;
...
impl Qwen3TtsTalker {
    ...
    fn text_embed_checked(&self, ids: &[u32], label: &str) -> Result<Tensor> {
        ...
        Ok(self
            .text_projection
            .forward(&self.model.text_embedding.forward(&ids_tensor(ids, &self.device)?)?)?)
    }

    fn codec_embed_checked(&self, ids: &[u32], label: &str) -> Result<Tensor> {
        ensure_ids_in_range(ids, self.cfg.vocab_size, label)?;
        Ok(self.model.codec_embedding.forward(&ids_tensor(ids, &self.device)?)?)
    }
    ...
    fn codec_embed(&self, ids: &[u32]) -> candle_core::Result<Tensor> {
        self.model.codec_embedding.forward(&ids_tensor(ids, &self.device)?)
    }
    ...
    fn build_icl_prompt_embeddings(
        ...
        if prepared.input_ids.len() < 8 || ref_ids.len() < 5 {
            anyhow::bail!("Qwen3-TTS ICL prompt received unexpectedly short input/ref token sequences.");
        }
        ...
        if non_streaming_mode {
            ...
            return Ok((Tensor::cat(&[&icl_input_embed, &codec_embed.broadcast_add(&codec_pad)?], 1)?, tts_pad_embed.clone()));
        }
        ...
        if text_lens > codec_lens {
            ...
            let text_embed = if codec_lens > text_lens {
                let pad = tts_pad_embed.expand((1, codec_lens - text_lens, self.cfg.hidden_size))?;
                Tensor::cat(&[&text_embed, &pad], 1)?
            } else {
                text_embed
            };
            Ok((text_embed.broadcast_add(&codec_embed)?, tts_pad_embed.clone()))
        }
    }
    ...
    fn speaker_embed_from_request(
        ...
        Ok(Some(
            self.codec_embed_checked(ids, "speaker codec")?
    ...
    fn resolve_language_id(&self, prepared: &Qwen3TtsPreparedRequest) -> Result<Option<u32>> {
        ...
        let language_id = self
            ...
            .get(&normalized)
            .copied()
            .ok_or_else(|| anyhow::anyhow!("Unsupported Qwen3-TTS language `{}`.", prepared.language))?;
        Ok(Some(language_id))
    }
    ...
    fn build_prompt_embeddings(
        ...
        let codec_input_embedding_0 =
            self.codec_embed_checked(&codec_prefill, "codec prefill")?;
        ...
        let role_raw = self.model.text_embedding.forward(&ids_tensor(
            &prepared.input_ids[..3],
            &self.device,
        )?)?;
        ...
        let pad_prefix = tts_pad_embed.expand((1, codec_input_embedding.dim(1)? - 2, self.cfg.hidden_size))?;
        let talker_prefix = Tensor::cat(&[&pad_prefix, &tts_bos_embed], 1)?
            .broadcast_add(&codec_input_embedding.narrow(1, 0, codec_input_embedding.dim(1)? - 1)?)?;
            ...
            && prepared.requires_ref_codes
        {
            ...
            let text_first_raw = self.model.text_embedding.forward(&ids_tensor(
                &prepared.input_ids[3..4],
                &self.device,
            )?)?;
            ...
            let text_first = self
                ...
                .broadcast_add(&codec_input_embedding.narrow(1, codec_input_embedding.dim(1)? - 1, 1)?)?;
            ...
            if non_streaming_mode {
                ...
                talker_input_embed = talker_input_embed.narrow(1, 0, talker_input_embed.dim(1)? - 1)?;
                let prompt_tail = Tensor::cat(
                    &[
                        &text_embed.broadcast_add(&text_pad)?,
                        &tts_eos_embed.broadcast_add(
                            &self.codec_embed_checked(&[self.cfg.codec_pad_id], "tts eos codec pad")?,
                        )?,
                        &tts_pad_embed.broadcast_add(
                            &self.codec_embed_checked(&[self.cfg.codec_bos_id], "tts pad codec bos")?,
                        )?,
                    ],
                    1,
                )?;
        ...
        let (prompt_last_mean, prompt_last_std, prompt_last_head8) =
            tensor_stats_preview(&talker_input_embed.narrow(1, talker_input_embed.dim(1)? - 1, 1)?)?;
        tracing::info!(
    ...
    ) -> Result<(usize, f32, f32, Option<usize>, f32, bool, f32, f32, Option<usize>)> {
    ...
    pub fn generate_codes(
        ...
        for step in 0..max_new_tokens {
            ...
            let first_logits = self.codec_head.forward(&last_hidden)?.squeeze(0)?.squeeze(0)?;
            if step == 0 {
                tracing::info!(
            ...
            let mut frame_embeds = vec![self.codec_embed_checked(&[first_code], "generated first codec")?];
            ...
            for group_idx in 0..self.cfg.num_code_groups - 1 {
                ...
                if group_idx + 1 < self.cfg.num_code_groups - 1 {
                    predictor_hidden = self
                        .code_predictor
                        .forward_embeds_cached(
                            embed,
                            &mut predictor_cache,
        ...
        let out = Tensor::from_vec(
            codes,
            (1, frames, self.cfg.num_code_groups),
            &self.device,
        )?;
        tracing::info!(
